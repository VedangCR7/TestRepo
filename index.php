...
$vegRestaurant = "SELECT * from user where usertype='Restaurant' and is_active=1 and (restauranttype='veg' or restauranttype='both') AND id NOT IN(85,101,153,154) ORDER BY id DESC";
$query = mysqli_query($conn, $vegRestaurant);
...
<body style="background-color:white;">
	...
	<?php 
		$countries = "SELECT country FROM user WHERE country != '' AND is_active = '1'";
		$query = mysqli_query($conn, $countries);
		$country = [];

		if(mysqli_num_rows($query) > 0){
			while ($row = mysqli_fetch_assoc($query)) {
				array_push($country, strtolower($row['country']));
			}
		}
		...
		<div class="main-container">
			<div class="container">
				<div class="row">
					...
					<div class="col-md-4 col-sm-4 col-md-12 col-12 py-3">
						<form action="" method="post">
							<!-- <button class="btn btn-default show_all_rest" style="float:right;margin-bottom: 10px;">View All</button> -->
							<select class="form-control m-3 d-block" id="filter" style="float:right;margin-bottom: 10px;" >
								<option value="">Restaurant Type</option>
								<option value="all">All Restaurant </option>
								<option value="veg"> Veg Restaurant </option>
								<option value="nonveg"> Non Veg Restaurant</option>
							</select>
							<button type="button" class="btn btn-success"  id="submit">Filter</button>
						</form>
					</div>
				</div> 
				<?php if(empty($_GET['veg']) && empty($_GET['page']) && empty($_GET['nonveg']) ){  ?>
					<!-- <div class="row"> -->
	...
	<?php
		if(isset($_GET['page']) || isset($_GET['veg']) || isset($_GET['nonveg']))
		{
			$showRecordPerPage = 8;
			...
			elseif(isset($_GET['veg']) && !empty($_GET['veg']))
			{
				$currentPage = $_GET['veg'];
			}
			elseif (isset($_GET['nonveg']) && !empty($_GET['nonveg'])){
				$currentPage = $_GET['nonveg'];
			}else {
				$currentPage = 1;
			}
			...
			if (isset($_GET['veg'])) {
				$totalvenSQL = "SELECT * from user where usertype='Restaurant' and is_active= '1' and  restauranttype='veg'  AND  id NOT IN(85,101,153,154) ORDER BY id DESC";
			}

			if (isset($_GET['nonveg'])) {
				$totalvenSQL = "SELECT * from user where usertype='Restaurant' and is_active= '1' and restauranttype='nonveg' AND  id NOT IN(85,101,153,154) ORDER BY id DESC";
			}

			// if (isset($_GET['nonveg'])) {
			// 	$totalvenSQL = "SELECT * FROM user WHERE is_active = '1' AND usertype='Restaurant' and  restauranttype='nonveg' AND id NOT IN(85,101,153,154)  ORDER BY id DESC";
			// }
	...
	?>	
			<div class="main-container show_pagination_all_restaurant" style="padding-top: 7px;">
				<div class="container">
					...
					if (isset($_GET['veg'])) {
						$sql = "SELECT * FROM user WHERE is_active = '1' AND usertype='Restaurant' AND restauranttype='veg' AND  id NOT IN(85,101,153,154) ORDER BY id DESC LIMIT $startFrom, $showRecordPerPage";
					}

					if (isset($_GET['nonveg'])) {
						$sql = "SELECT * FROM user WHERE is_active = '1' AND usertype='Restaurant' AND restauranttype='nonveg'  AND id NOT IN(85,101,153,154) ORDER BY id DESC LIMIT $startFrom, $showRecordPerPage";
					}
						...
						<div class="col-md-12 tp-pagination">
							...
							<ul class="pagination">
								<li>
									...
									<a href="?<?php echo $index; ?>=<?php echo $previousPage ?>" aria-label="Previous"> <span aria-hidden="true">Previous</span> </a>
								</li>
								
								<?php if($currentPage != $firstPage) { ?>
								<li class="page-item">
									<a class="page-link" href="?<?php echo $index; ?>=<?php echo $firstPage ?>" tabindex="-1" aria-label="Previous">
										<span aria-hidden="true">First</span>           
									</a>
								</li>
								<?php } ?>
								<?php if($currentPage >= 2) { ?>
									<li class="page-item"><a class="page-link" href="?<?php echo $index; ?>=<?php echo $previousPage ?>"><?php echo $previousPage ?></a></li>
								<?php } ?>
								<li class="page-item active"><a class="page-link" href="?<?php echo $index; ?>=<?php echo $currentPage ?>"><?php echo $currentPage ?></a></li>
								<?php 
								if($currentPage != $lastPage) 
								{
									...
									for($frstPage1=$currentPage; $frstPage1<$lastPage-1;$frstPage1++)
									{
										if($i<10)
										{
											if($frstPage0==0)
											{
												...
												$frstPage0++;
												?>
												<li class="page-item"><a class="page-link" href="?<?php echo $index; ?>=<?php echo $nextPage; ?>"><?php echo $nextPage ?></a></li>
												<?php
											}
											else
											{
									...
									<li class="page-item">
										<a class="page-link" href="?<?php echo $index; ?>=<?php echo $nextPage; ?>" aria-label="Next">
	...
	?>
		<?php 
			$labels = ["Veg", "Non Veg", "Place"];
			$country = array_unique($country);
			$toShow = $labels[array_rand($labels)];

			$sql = "SELECT * FROM user u, (select max(id) * rand() randid from user ) b WHERE  is_active = 1 AND id NOT IN(85,101,153,154) AND ";
			if ($toShow == "Veg") {
				$sql .= "restauranttype = 'veg' LIMIT 5";
			}

			if ($toShow == "Place") {
				$toShow = $country[array_rand($country)];
				// echo $toShow;
				$sql .= "country = '$toShow'  LIMIT 5";
			}

			if ($toShow == "Non Veg") {
				$sql .= "restauranttype ='nonveg' LIMIT 5";
			}
			...
			if(mysqli_num_rows($querySql) > 0){
				while ($row = mysqli_fetch_assoc($querySql)) {
					...
					array_push($recomendedResult, $data);
				}
			}
			$countRestaurant = count($recomendedResult);
		?>	
		...
		<div class="container">
				...
				<div class="col-12">
					<?php 
						if ($countRestaurant > 0) {
							?>
								<h3 style="font-weight:bold;"><?php echo $countRestaurant . " " . ucwords($toShow);?> Recommended Restaurant(s) </h3>
								<?php
						}else{
							?>
								<h3 style="font-weight:bold;">No recomended Restaurants found for <?php echo $toShow; ?> </h3>
							<?php
						}
					?>
				</div>
			...
			<div class="row" style="padding: 10px 0 10px 0;">
				<?php
					if (count($recomendedResult) < 1) {
						echo "<h3> Recommedation for {$toShow} Restaurant not available </h3>";
					}else{
						foreach ($recomendedResult as $result) {
							?>
								<div class="col-md-3 vendor-box">
									<div class="vendor-image" style="border: 1px solid #e9e6e0;">
										<a href="restaurants/<?php echo $result['city']?str_replace(" ","_",$result['city']):'';?>/<?=$result['b_name']?str_replace(" ","-",$result['b_name']):''?>">
											<?php if($result['img'] == 'assets/images/users/user.png'){ ?>
											<img src="<?='https://d24h2kiavvgpl8.cloudfront.net/profile/default_restaurant.png'?>" alt="youcloud restaurant" class="img-responsive" style="height: 147px; width: 263px;">
											<?php }else { ?>
											<img src="<?=$result['img']?>" alt="youcloud restaurant" class="img-responsive" style="height: 147px; width: 263px;">
											<?php } ?>
										</a>
									</div>
									<div class="vendor-detail" style="height:160px;">
										<div class="caption" style="padding: 11px;">
											<h2><a href="restaurants/<?php echo $result['city'] ? str_replace(" ","_",$result['city']):'';?>/<?=$result['b_name'] ? str_replace(" ","-",$result['b_name']):''?>" class="title"><?=$result['b_name']?></a></h2>
											<p class="location" style="margin-bottom: 0px!important;max-height:80px;overflow:hidden;" title="<?php echo ucwords(strtolower($result['address'])); ?>">
												<?php 
													if(($result['address'] !='' || $result['city'] != ''))
													{
														?>
															<i class="fa fa-map-marker"></i>
														<?php 
														if(strlen($result['address']) > 22) { echo substr(strtolower($result['address']), 0 , 22)."..."; }
														else { echo ucwords(strtolower($result['address'])); }
													}
												?>
											</p>
											<p class="location"><?php echo ucwords(strtolower($result['city'])); ?></p>
										</div>
									</div>
								</div>
							<?php
			...
			<div class="container">
				<div class="row">
					<div class="col-md-offset-2 col-md-8 parallax-caption">
						...
						<a href="<?php echo $base_url.'admin/register'; ?>" target="_blank" class="btn btn-default" style="margin-top:20px;background-color:white;color:black;line-height:50px;">Get Started <i class="fa fa-play-circle"></i></a> </div>
				</div>
			</div>
			</div>
		</section>
							...
							<div class="row">
								<div class="col-lg-12 col-md-12 col-sm-12 col-12">
									...
									<p>QR Menu ensure Reducing Physical contact and helps your business to apply physical distancing rules.
										QR Menu comes with features  such as Increase efficiency, Cost reduction, Time saving and Customer experience enhancement.
									</p>
								</div>
							</div>
						</div>
						...
						<div class="col-lg-11 col-md-11 col-sm-8 col-8">
							<div class="row">
								<div class="col-lg-12 col-md-12 col-sm-12 col-12">
									...
									<p>Running a restaurant is no easy management. You have a never-ending list of tasks, from managing menus pricing, table managment and checking in on the kitchen. 
										Whether you own a fast food restaurant, fine dining establishment,  restaurant management system to get the most out of your restaurant business.
									</p>
								</div>
							</div>
						</div>
						...
						<div class="row">
							...
							<h4 style="font-weight:bold;">Waiting List Management</h4>
								<p>Improve how your restaurant manages your waitlist, while enhancing your table management and optimizing your customers reservation process.
									Real-time view, SMS alerts, Call from phone. Capture reservation data through your web panel on your phone.
								</p>
							</div>
						</div>
						</div>
						...
						<div class="col-lg-11 col-md-11 col-sm-8 col-8">
							<div class="row">
								<div class="col-lg-12 col-md-12 col-sm-12 col-12">
									...
									<p>Take orders on mobile and send to kitchen by simply creating captain pad logins.  Optionally use the Captain pad to accept orders from table, block or cancel table ordering. Assign all tables or limited tables to each captain pad for more efficiency.
									</p>
								</div>
							</div>
						</div>
