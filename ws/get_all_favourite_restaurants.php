<?php
	...
	if(isSet($_REQUEST['user_id']) && isSet($_REQUEST['latitude']) && isSet($_REQUEST['longitude']))	
	{
		...
		if($count0 > 0)	
		{		
			$getFavRecQry = "SELECT f.*, r.* FROM Favourites f, user r WHERE f.account_id='$user_id' AND f.restaurant_id=r.id ORDER BY fav_id DESC";
			...
			if($getFavRecCnt > 0)
			{
				...
				while($getFavRecRw = mysqli_fetch_assoc($getFavRecRes))
				{
					...
					$address = stripslashes($getFavRecRw['address']);
					...
					$restaurants_list[] = array('restaurant_id'=>$resto_id,'restaurant_name'=>$resto_name,'business_name'=>$business_name,'restaurant_address'=>$address,'restaurant_city'=>$restoCity,'distance_from_postal_code'=>$distinkm,'ratings'=>'4.5','contact_numbar'=>$contact_numbar,'restaurant_image'=>$resto_img,'restaurant_type'=>ucfirst($restauranttype),'offers'=>'Upto 50% off','food_type'=>'Continental food');
				}
				$show = array('result'=>'success','msg'=>'Favourite Restaurants found.','restaurants_list'=>$restaurants_list);
				echo json_encode($show);
			}
			else
			{
				$show = array('result'=>'failed', 'msg'=>'Favourite Restaurant not found.');	
				echo json_encode($show);
			}
		}
		else{
