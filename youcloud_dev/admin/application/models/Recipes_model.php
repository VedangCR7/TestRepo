...
class Recipes_model extends My_Model {
    ...
    var $recipe_image;
    var $thumb_path;
    var $is_for_restaurant;
    var $isadded_for_restaurant;
    var $best_time_to_eat;
    var $ref_recipe_id;
    var $is_menu_fromrestaurant;
    var $quantity;
    var $price1;
    var $quantity1;
    var $price2;
    ...
    var $price3;
    var $quantity3;
    var $is_todays_special;
    var $is_bar_menu;
    var $main_menu_id;
    var $is_recipe_active;
    var $recipe_image_id;
    ...
    var $fields=array('name','price','recipe_type','notes','quantity_per_serving','weight_loss','serving_size','total_weight','pack_size','batch_size','group_id','alacal_recipe_id','is_delete','logged_user_id','views','subscription_id','is_active','recipe_image','declaration_name','is_for_restaurant','isadded_for_restaurant','ref_recipe_id','is_menu_fromrestaurant','best_time_to_eat','quantity','price1','quantity1','price2','quantity2','price3','quantity3','is_todays_special','is_bar_menu','main_menu_id','is_recipe_active','recipe_image_id','description','ingredients_name','thumb_path','product_code','is_addon_menu','recipe_date');
    public function __construct()
    {
      ...
      // added by victor
    public function saveOldPrice($data){
        $query = $this->db->insert('old_price_log', $data );
        if ($query) {
            return "success";
        }else{
            return "error";
    ...
    public function getPriceSalesCount($id, $from , $to){
        $this->db->select("*");
        $this->db->from("old_price_log l");
        $this->db->where("l.recipe_id", $id);
        $this->db->where("DATE_FORMAT(date_created, '%Y-%m-%d') BETWEEN '$from' AND '$to'");
        ...
        while ($j <= $queryCount) {
            for ($k=$i; $k < $j; $k++) { 
                $nextIndex = $k+1;
                if ($nextIndex >= $queryCount) {
                    $this->db->where("recipe_id", $id);
                    $this->db->where("price", $query[$k]['price']);
                    $this->db->where("created_at >=", $query[$k]['date_created']);
                    $results = $this->db->get('order_items')->result_array();
                }else{
                    $this->db->where("recipe_id", $id);
                    $this->db->where("price", $query[$k]['price']);
                    $this->db->where("created_at >=", $query[$k]['date_created']);
                    $this->db->where("created_at <=", $query[$k+1]['date_created']);
                    $results = $this->db->get('order_items')->result_array();
                }
    ...
    public function list_recipes_typahead($tablecat_id="")
	{
        if($tablecat_id==""){
            $this->db->select("r.id,r.group_id,r.name,r.price,r.recipe_type,r.product_code,ao.discount,ao.status as offer_status,ao.discount_type");
		...
		{
            $this->db->select("r.id,r.group_id,r.name,ifnull(rp.price,r.price) as price,r.recipe_type,r.product_code,ao.discount,ao.status as offer_status ,ao.discount_type",FALSE);
            $this->db->from('recipes r');
    ...
    public function list_recipes_typahead1(){
        $this->db->select("r.id,r.group_id,r.name,r.price");
        $this->db->from('recipes r');
        $this->db->join('menu_group m','m.id=r.group_id');
        $this->db->join('menu_master mm','mm.id=r.main_menu_id');
        $this->db->where('r.logged_user_id',$_SESSION['user_id']);
    ...
    public function list_addon_typahead(){
        $this->db->select("id,menu_group_id,addon_name as name");
        $this->db->from('addon_menu');
    ...
    public function get_recipe($id){
        $this->db->select("r.*,m.title as group_name,mm.name as menu_master_name");
        $this->db->from('recipes r');
       ...
       /* $this->db->join('recipe_images_master im','im.id=r.recipe_image_id','left');*/
        ...
        $row['nutrition']=$this->recipe_nutritient_model->list_nutrients($id);
        $row['prices']=$this->recipe_price_model->list_recipe_prices($id);
        return  $row; 
    }
    
    public function get_offer_recipe($offer_id,$tablecat_id=""){
        if($tablecat_id!=""){
            $this->db->select("r.*,a.id as offer_id,a.offer_image,a.discount,a.title,a.description,ifnull(rp.price,r.price) as price",FALSE);
            $this->db->from('admin_offer a');
            $this->db->join('recipes r','r.id=a.recipe_id');
            $this->db->join('recipe_prices rp','rp.recipe_id=r.id AND rp.table_category_id='.$tablecat_id,'left');
            $this->db->where('a.id',$offer_id);
            $query = $this->db->get();
            $row = $query->row_array();
        }else{
            $this->db->select("r.*,a.id as offer_id,a.offer_image,a.discount,a.title,a.description");
            $this->db->from('admin_offer a');
            $this->db->join('recipes r','r.id=a.recipe_id');
            $this->db->where('a.id',$offer_id);
            $query = $this->db->get();
    ...
    public function get_recipe_withoutingredients($id,$tablecat_id=""){
        if($tablecat_id!=""){
            $this->db->select("r.*,ifnull(rp.price,r.price) as price",FALSE);
            $this->db->from('recipes r');
            $this->db->join('recipe_prices rp','rp.recipe_id=r.id AND rp.table_category_id='.$tablecat_id,'left');
           /* $this->db->join('recipe_images_master im','im.id=r.recipe_image_id','left');*/
            $this->db->where('r.id',$id);
    ...
    public function get_recipe_with_ingredients($id){
        $this->db->select("r.*,m.title as group_name");
        $this->db->from('recipes r');
        $this->db->join('menu_group m','m.id=r.group_id','left');
       /* $this->db->join('recipe_images_master im','im.id=r.recipe_image_id','left');*/
        $this->db->where('r.id',$id);
...
/*
    ...
    public function update_alacalc_recipe($aalcalc_recipe_id,$recipe_id)
	{
        $declaration_names=$this->aalcalc_model->get_recipe_declarationname($aalcalc_recipe_id);
		
		if(isset($declaration_names) && $declaration_names!="")
		{
			$r=$this->recipes_model;
        ...
        $recipe_details=$this->aalcalc_model->get_recipe($aalcalc_recipe_id);

        $linked_data=$recipe_details['linked'];
		
        $nutrition=$linked_data['nutrition'];
		
        /* echo "<pre>";
        print_r($nutrition);echo '<br>';
        die(); */       
		
		if($nutrition)
		{
			/* Delete recipe nutrition data */
			
			$this->db->where('recipe_id', $recipe_id);
			$this->db->delete('recipe_nutritient');
	
            foreach ($nutrition as $key => $row) 
			{
                $rn=$this->recipe_nutritient_model;
                /* $status=$rn->isexist_row(array('recipe_id'=>$recipe_id,'name'=>$key));
        ...
        $allergens=$linked_data['allergens'];
		/* echo "<pre>";
        print_r($allergens);
        die; */
		if($allergens)
		{
			...
			{
                $rn=$this->allergens_model;
                $status=$rn->isexist_row(array('title'=>$value));
                
				if($status!="not")
				{
                    $r=$this->recipe_allergens_model;
                    $r_status=$r->isexist_row(array('allergens_id'=>$status,'recipe_id'=>$recipe_id));
                   
					if($r_status=="not")
					{
                        $r->allergens_id=$status;
				...
				else
				{
                    $rn=$this->allergens_model;
                    $rn->title=$value;
                    $rn->image_url="assets/images/allergens/contraversal.png";
                    $allergens_id=$rn->add();

                    $r=$this->recipe_allergens_model;
                    $r_status=$r->isexist_row(array('allergens_id'=>$allergens_id,'recipe_id'=>$recipe_id));
                    
					if($r_status=="not")
					{
                        $r->allergens_id=$allergens_id;
                        $r->recipe_id=$recipe_id;
                        $r->add();
                    }
					else
    ...
    public function list_todays_offers($user_id,$main_menu_id,$tablecat_id="",$recipetype=''){
        if($tablecat_id!=""){
             $this->db->select("r.*,a.id as offer_id,a.offer_image,a.discount,a.title,a.description,a.start_date,a.end_date,a.discount_type,DATE_FORMAT(recipe_date,'%d %M %Y') as recipe_date,ifnull(rp.price,r.price) as price",FALSE);
            $this->db->from('admin_offer a');
            $this->db->join('recipes r','r.id=a.recipe_id');
            $this->db->join('menu_group m','m.id=r.group_id');
            $this->db->join('menu_master ms','ms.id=r.main_menu_id');
            $this->db->join('recipe_prices rp','rp.recipe_id=r.id AND rp.table_category_id='.$tablecat_id,'left');
            $this->db->where('r.is_delete',0);
            $this->db->where('r.is_active',1);
            ...
            $this->db->where('a.restaurant_id',$user_id);
            $this->db->where('r.main_menu_id',$main_menu_id);
			if($recipetype!=''){
				$this->db->where('r.recipe_type',$recipetype);
			}
            ...
            $this->db->select("r.*,a.id as offer_id,a.offer_image,a.discount,a.title,a.description,a.start_date,a.end_date,a.discount_type,DATE_FORMAT(recipe_date,'%d %M %Y') as recipe_date");
            $this->db->from('admin_offer a');
            $this->db->join('recipes r','r.id=a.recipe_id');
            $this->db->join('menu_group m','m.id=r.group_id');
            $this->db->join('menu_master ms','ms.id=r.main_menu_id');
    ...
    public function list_todays_special_userwise($user_id,$main_menu_id,$tablecat_id="",$recipetype=""){
        if($tablecat_id!=""){
             $this->db->select("r.*,a.id as offer_id,a.offer_image,a.discount,a.title,a.description,a.start_date,a.end_date,a.discount_type,DATE_FORMAT(recipe_date,'%d %M %Y') as recipe_date,ifnull(rp.price,r.price) as price",FALSE);
            $this->db->from('recipes r');
            $this->db->join('menu_group m','m.id=r.group_id');
            $this->db->join('menu_master ms','ms.id=r.main_menu_id');
            $this->db->join('recipe_prices rp','rp.recipe_id=r.id AND rp.table_category_id='.$tablecat_id,'left');
			$this->db->join('admin_offer a','a.recipe_id=r.id','left');
            $this->db->where('r.is_delete',0);
            $this->db->where('r.is_active',1);
            $this->db->where('r.is_recipe_active',1);
   ...
   public function make_active($id){    
    ...
    public function list_recipes($page_no,$records_per_page,$group_id="",$main_menu_id="",$searchkey="")
	{
        if($group_id!="" && $main_menu_id!="")
            $recipes_count=$this->db->select('count(*) as cnt')->from('recipes')->where('logged_user_id',$_SESSION['user_id'])->where('is_delete',0)->where('is_active',1)->where('group_id',$group_id)->where('main_menu_id',$main_menu_id)->get()->row_array();
        else if($group_id=="" && $main_menu_id!="")
            $recipes_count=$this->db->select('count(*) as cnt')->from('recipes')->where('logged_user_id',$_SESSION['user_id'])->where('is_delete',0)->where('is_active',1)->where('main_menu_id',$main_menu_id)->get()->row_array();
        else if($group_id!="" && $main_menu_id=="")
            $recipes_count=$this->db->select('count(*) as cnt')->from('recipes')->where('logged_user_id',$_SESSION['user_id'])->where('is_delete',0)->where('is_active',1)->where('group_id',$group_id)->get()->row_array();
        else
            $recipes_count=$this->db->select('count(*) as cnt')->from('recipes')->where('logged_user_id',$_SESSION['user_id'])->where('is_delete',0)->where('is_active',1)->get()->row_array();
		...
		else
		{
            if($records_per_page=="all")
			{
				...
				if($searchkey!="")
				{
					if(is_numeric($searchkey)==1)
					{
						$this->db->like('lower(r.product_code)',strtolower($searchkey));
					}
					else
					{
						$this->db->like('lower(r.name)',strtolower($searchkey));
    ...
    public function list_recipes_for_new($page_no,$records_per_page,$searchkey="")
	{
        $recipes_count=$this->db->select('count(*) as cnt')->from('recipes')->where('logged_user_id',$_SESSION['user_id'])->where('is_delete',0)->where('main_menu_id',null)->get()->row_array();
        ...
        if($recipes_count['cnt']<=0)
		{
           ...
           /* $this->db->join('recipe_images_master im','im.id=r.recipe_image_id','left');*/
            $this->db->where('r.is_delete',0);
            $this->db->where('r.main_menu_id',null);
	...
	public function is_productcode_exists($product_code)
	{
		$this->db->select("*");
		...
		$product_code_data = $query->row_array();	
        return  $product_code_data;
    }

    public function list_addon_rcipes($page_no,$records_per_page,$searchkey=""){
        $addon_count=$this->db->select('count(*) as cnt')->from('addon_menu')->where('restaurant_id',$_SESSION['user_id'])->where('is_delete',0)->get()->row_array();
      
        if($addon_count['cnt']<=0){
            $this->db->select('am.*,mg.title,r.name,count(amo.addon_menu_id) as option_count');
                $this->db->from('addon_menu as am');
                $this->db->join('menu_group as mg','mg.id =am.menu_group_id','left');
				$this->db->join('recipes as r','r.id =am.menu_id','left');
                $this->db->join('addon_menu_option as amo','amo.addon_menu_id =am.id','left');
                $this->db->where('am.restaurant_id',$_SESSION['user_id']);
                $this->db->where('am.is_delete',0);
                $this->db->where('amo.is_delete',0);
                $this->db->group_by('amo.addon_menu_id');
            ...
            return  array(
                'manager'=>$result,
                'total_count'=>$addon_count['cnt'],
                'total_pages'=>1,
                'page_no'=>1,
                'page_total_text'=>"0 - ".$addon_count['cnt']
            ); 
        }else{
            if($records_per_page=="all"){
                $this->db->select('am.*,mg.title,r.name,count(amo.addon_menu_id) as option_count');
                ...
                $this->db->order_by("am.id DESC");
                if($searchkey!="")
                    $this->db->like('lower(am.addon_name)',strtolower($searchkey));
                    $this->db->where('am.restaurant_id',$_SESSION['user_id']);
                    $this->db->where('am.is_delete',0);
                    $this->db->where('amo.is_delete',0);
                    $this->db->group_by('amo.addon_menu_id');
                ...
                return  array(
                    ...
                    'total_pages'=>1,
                    'page_no'=>1,
                    'page_total_text'=>"1 - ".$addon_count['cnt']
                ); 
            }else{
               ...
               /* echo $offset;
                ...
                $result = $query->result_array();
                $total_pages = ceil($addon_count['cnt'] / $records_per_page);

                if($addon_count['cnt']<$records_per_page)
                    $to_page=$addon_count['cnt'];
                else
                    $to_page=$records_per_page;
                return  array(
                    'manager'=>$result,
	...
	public function is_exits($title,$main_menu_id,$product_code)
	{
        $this->db->select("*");
        $this->db->from('recipes');
	...
	public function is_productcod_exits($product_code)
	{
        $this->db->select("*");
        $this->db->from('recipes');
        $this->db->where('product_code',$product_code);
	...
	public function importdata($data) 
	{
		...
		if(!empty($data))
		{
			...
			{
				//echo $j;
				/* $this->waiting_manager_model->insert_any_query('recipe_addon',['recipe_id'=>$recipe_id,'addon_id'=>$addon_id_array1[$i]]); */
			
				$addon_id_array = $data[$j]['recipe_addon_id'];												
				$menu_group_id=$data[$j]['group_id'];
				$product_code=$data[$j]['product_code'];
				
				if($product_code!="")
				{
					$this->db->select("*");
					$this->db->from('recipes');
					$this->db->where('product_code',$data[$j]['product_code']);
					$this->db->where('logged_user_id',$_SESSION['user_id']);
					$query = $this->db->get();
					$row = $query->row_array();
					...
					{/* echo 'one';exit; */
						...
						$r->name=$data[$j]['title'];
						$r->description=$data[$j]['description'];
						$r->price=$data[$j]['price'];
						$r->quantity='100';
						$r->recipe_type=$data[$j]['recipe_type'];
						$r->main_menu_id=$data[$j]['main_menu_id'];
						$r->group_id=$data[$j]['group_id'];;
						$r->recipe_date=$data[$j]['recipe_date'];
						$r->logged_user_id=$_SESSION['user_id'];
						$r->is_active=$data[$j]['is_active'];
						$r->best_time_to_eat=$data[$j]['best_time_to_eat'];
						$r->is_addon_menu=$data[$j]['is_addon'];
						$r->product_code=$data[$j]['product_code'];
						$r->is_active=$data[$j]['is_active'];
						$r->is_recipe_active=$data[$j]['is_active'];
						$r->recipe_image=$data[$j]['recipe_image'];
						
						$recipe_id=$r->add();
	  
						if($data[$j]['table_category']!="")
						{
							$rp=$this->recipe_price_model;
							$rp->recipe_id=$recipe_id;
							$rp->price=$data[$j]['price'];
							$rp->table_category_id=$data[$j]['table_category'];
							$rp->is_default=1;
							$rp->add();
						}
						
						if($data[$j]['table_category1']!="")
						{
							$rp=$this->recipe_price_model;
							$rp->recipe_id=$recipe_id;
							$rp->price=$data[$j]['price1'];
							$rp->table_category_id=$data[$j]['table_category1'];
							$rp->is_default=1;
							$rp->add();
						}
						
						if($data[$j]['table_category2']!="")
						{
							$rp=$this->recipe_price_model;
							$rp->recipe_id=$recipe_id;
							$rp->price=$data[$j]['price2'];
							$rp->table_category_id=$data[$j]['table_category2'];
							$rp->is_default=1;
							$rp->add();
						}
						
						if($data[$j]['table_category3']!="")
						{
							$rp=$this->recipe_price_model;
							$rp->recipe_id=$recipe_id;
							$rp->price=$data[$j]['price3'];
							$rp->table_category_id=$data[$j]['table_category3'];
							$rp->is_default=1;
							$rp->add();
						}
						
						if($data[$j]['table_category4']!="")
						{
							$rp=$this->recipe_price_model;
							$rp->recipe_id=$recipe_id;
							$rp->price=$data[$j]['price4'];
							$rp->table_category_id=$data[$j]['table_category4'];
							$rp->is_default=1;
							$rp->add();
						}
						
						if($data[$j]['table_category5']!="")
						{
							$rp=$this->recipe_price_model;
							$rp->recipe_id=$recipe_id;
							$rp->price=$data[$j]['price5'];
							$rp->table_category_id=$data[$j]['table_category5'];
							$rp->is_default=1;
							$rp->add();
						}
						
						if($data[$j]['table_category6']!="")
						{
							$rp=$this->recipe_price_model;
							$rp->recipe_id=$recipe_id;
							$rp->price=$data[$j]['price6'];
							$rp->table_category_id=$data[$j]['table_category6'];
							$rp->is_default=1;
							$rp->add();
						}				
						...
						if($addon_id_array!="")
						{
							$addon_id_array1= explode(',',$addon_id_array);
							
							for ($i=0; $i < count($addon_id_array1); $i++) 
							{ 
								$this->waiting_manager_model->insert_any_query('recipe_addon',['recipe_id'=>$recipe_id,'addon_id'=>$addon_id_array1[$i]]);
							}
						}
