...
class Menu_group_model extends My_Model {
    ...
    var $fields=array('title','is_active','logged_user_id','sequence','main_menu_id','image_path','available_in','datetime','logged_user_id');
	...
	public function is_exits($title,$main_menu_id)
	{
        $this->db->select("*");
    ...
    public function check_group_name($group_name,$user_id,$group_id="",$main_group_id=""){
        $this->db->select("*");
        $this->db->from('menu_group');
		...
		if($main_group_id!="")
            $this->db->where('main_menu_id=',$main_group_id);
        $query = $this->db->get();
        $row = $query->row_array();
        return  $row;       
    }
    ...
    public function get_user_menugroups($user_id,$main_menu_id,$recipetype='')
	{
    ...
    public function loadgroup_recipes_formob($user_id,$group_id,$recipe_type="",$tablecat_id=""){
        if($tablecat_id!=""){
            ...
            $this->db->select("r.*,ao.discount,ao.discount_type,ao.status as offer_status,ifnull(rp.price,r.price) as price,ifnull(best_time_to_eat,'') as best_time_to_eat",FALSE);
           ...
           /* $this->db->join('recipe_images_master im','im.id=r.recipe_image_id','left');*/
            ...
            if($recipe_type!="")
                $this->db->where('r.recipe_type',$recipe_type);
            $query = $this->db->get();
            $result = $query->result_array();
        }else{
            $this->db->select('r.*,ao.discount,ao.discount_type,ao.status as offer_status,ifnull(best_time_to_eat,"") as best_time_to_eat');
            $this->db->from('recipes r');
            $this->db->join('admin_offer ao','ao.recipe_id=r.id','left');
            /*$this->db->join('recipe_images_master im','im.id=r.recipe_image_id','left');*/
            $this->db->where('r.is_delete',0);
    ...
    public function search_recipes_formob($user_id,$main_menu_id,$recipe_type="",$searchkey="",$tablecat_id=""){
         if($tablecat_id!=""){
             $this->db->select("r.*,ao.discount,ao.discount_type,ao.status as offer_status,ifnull(rp.price,r.price) as price,ifnull(best_time_to_eat,'') as best_time_to_eat,rp.price as rp_price",FALSE);
            $this->db->from('recipes r');
    ...
    public function search_recipetype_formob($user_id,$main_menu_id,$searchkey="",$tablecat_id=""){
         if($tablecat_id!=""){
           ...
           /* $this->db->join('recipe_images_master im','im.id=r.recipe_image_id','left');*/
            ...
            $this->db->where('r.logged_user_id',$user_id);
            if($searchkey!="")
                $this->db->where('r.recipe_type',$searchkey);
            $query = $this->db->get();
            $result = $query->result_array();
        }else{
	...
	{
        $res = $this->db->insert_batch('menu_group',$data);
