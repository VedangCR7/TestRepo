var Menulist ={
    ...
    bind_events :function() {
        ...
        $(window).scroll(function(){
            ...
            Menulist.onScrollload(target);
        });

        var todays_special_count=Menulist.todays_special_count;
        if(todays_special_count<=0){
            var selected_div=$('.menu-section-onlyrecipes').first();
            Menulist.load_menu(selected_div);
        }else{
    ...
    updateCart:function(select_menu,input)
	{
		...
		var data={
            ...
            name:Menulist.capitalize_Words(select_menu.attr('name')),
            customer_id:Menulist.customer_id,
    ...
    onAddCart:function(){
        ...
        var data={
          ...
          name:Menulist.capitalize_Words($(this).attr('name')),
          customer_id:Menulist.customer_id,
    ...
    onScrollload:function(target){
        ...
        target.each(function(e){
            var self=$(this);
            var $main_menu_id=Menulist.main_menu_id;
            ...
            if(!self.find('.append-group-recipes').html() && $(window).scrollTop()!=0 ){
                if($.inArray(group_id,done_group)==-1){
                    ...
                    if(Math.ceil(diff)<100){
                        ...
                        if(Menulist.is_category_prices==1){
                            var data={
                                group_id :group_id,
                                user_id:$restid,
                                tablecategory_id:Menulist.tablecategory_id,
                        ...
                        $.ajax({
                            url: Menulist.base_url+"menus/list_recipes_ofgroup",
                            type:'POST',
                            dataType: 'json',
                            data: data,
                            success: function(result){
                                if (result.status) { 
                                    if(result.recipes){
                                        ...
                                        var html="";
                                        for(i in recipes){
                                            html+=Menulist.get_recipe_html(recipes[i],group_id,$restid,$main_menu_id);
                                        }
    ...
    listgroupItems:function(){
        ...
        if(!selected_div.find('.append-group-recipes').html()){
            ...
            if(Menulist.is_category_prices==1){
                var data={group_id :group_id,user_id:$restid,recipetype:$('#choose_recipetype').val(),tablecategory_id:Menulist.tablecategory_id}
            }
            else{
        ...
        if(search=="" && recipetype==""){
            ...
            if(Menulist.is_category_prices==1){
                var data={
                    ...
                    main_menu_id:Menulist.main_menu_id,
                    tablecategory_id:Menulist.tablecategory_id
            ...
            $.ajax({
                url: Menulist.base_url+"menus/search_recipes_formob",
                type:'POST',
                dataType: 'json',
                data: data,
                success: function(result){
                    if (result.status) { 
                        ...
                        var html="";
                        for(i in recipes){
                            html+=Menulist.get_recipe_html(recipes[i],recipes[i].group_id,restid,$main_menu_id);
                        }
    ...
    onChangelistview:function(){
        ...
        window.location.href= Menulist.base_url+'menus/'+main_menu+'/'+rest_id+'/'+table_id+'?recipetype='+$('#choose_recipetype').val()+'&list_view='+list_view_change+'&is_website='+is_website;
    },
    ...
    onChangerecipetype:function(){
        ...
        var table_id = $(this).attr('table-id');
        var is_website = $('#is_website').val();
        window.location.href= Menulist.base_url+'menus/'+main_menu+'/'+rest_id+'/'+table_id+'?recipetype='+$('#choose_recipetype').val()+'&list_view='+$('#show_view').val()+'&is_website='+is_website;
            // var selected_div=$('.menu-section-onlyrecipes').first();
            //     var next_group_id=selected_div.attr('group-id');
            //     if(next_group_id){
        ...
        //     $('.section-search-padding').find('.loader-div').show();
        //     $.ajax({
        //         url: Menulist.base_url+"menus/search_recipetype_formob",
        //         type:'POST',
        //         dataType: 'json',
        //         data: data,
        //         success: function(result){
                ...
                <div class="row">\
                    <div class="col-3 pr-0">\
                        <div class="col-12 p-0 justify-content-center">';
                            if(recipe['recipe_image']=="" || recipe['recipe_image']=="assets/images/users/menu.png")
                                var recipe_image=Menulist.base_url+"assets/images/users/menu.png";
                            else{
                            ...
                            if(recipe['recipe_type']=="veg") 
                             ...
                             if(tableid!=""){
                                        html+='<a href="'+Menulist.base_url+'menus/view/'+group_id+'/'+recipe['id']+'/'+$restid+'/'+tableid+'?is_website='+is_website+'"><div class="menu-img '+recipe_type+'" style="background-image:url('+recipe_image+');background-repeat: no-repeat;background-size: cover;background-position: center;">\
                            </div></a>';
                                    }else{
                                         html+='<a href="'+Menulist.base_url+'menus/view/'+group_id+'/'+recipe['id']+'/'+$restid+'/0"><div class="menu-img '+recipe_type+'" style="background-image:url('+recipe_image+');background-repeat: no-repeat;background-size: cover;background-position: center;">\
                            </div></a>';
                                    }
                            html+='</div>';
                       /* if($main_menu_id==1){*/
                    ...
                    <div class="col-9 pr-0">\
                        <div class="menu-txt">\
                            <h3>';
                            if(tableid!=""){
                                html+='<a href="'+Menulist.base_url+'menus/view/'+group_id+'/'+recipe['id']+'/'+$restid+'/'+tableid+'" class="food-recipe-name">'+Menulist.capitalize_Words(recipe['name'])+'</a>';
                            }else{
                                html+='<a href="'+Menulist.base_url+'menus/view/'+group_id+'/'+recipe['id']+'/'+$restid+'/0" class="food-recipe-name">'+Menulist.capitalize_Words(recipe['name'])+'</a>';
                            }
                            
                            html+='</h3>';
                            
                            html+='<ul class="menu-ingredients" recipe-id="'+recipe['id']+'">\
                                <li class="li-ingredients">';
                                if(recipe['ingredients_name']!="" && recipe['ingredients_name']!=null)
                                    html+=Menulist.capitalize_Words(recipe['ingredients_name']);
                                ...
                                html+='<li class="view-recipe float-right mb-2 '+mrclass+'" style="width:100%;margin-top:10px;">';
                                    html+='<span class="price-meta mb-0 float-left pricecss" style="color:#C70039;width:80%">';
                                if(recipe['price']=='Recipe Price' || recipe['price']==''){
                                    if($restid == 103){ 
                                        $price1 = 'MZN 0.00/-'; }
                                    else if($restid == 134){
                                        $price1 = '$ 0.00/-'; }
                                    else{ $price1= Menulist.currency_symbol+' 0.00/-'; }
                                }else{
                                    if(recipe['discount']!= null && recipe['offer_status'] == 1){
                                        ...
                                        if($restid == 103){ 
                                            ...
                                            $price1= Menulist.currency_symbol+' '+$discount_price+'/-'; }
                                    }
                                    else{
                                        ...
                                        if($restid == 103){ 
                                            ...
                                            $price1= Menulist.currency_symbol+' '+recipe['price']+'/-'; }
                                    }
                                }
                            ...
                            html+=$price1+'</span>';
                                    if(tableid!=""){
                                        if(is_website == ''){
                                        if(Menulist.is_customer_block==0 && autho_data.indexOf("Online order") !== -1 && autho_data.indexOf("Table Management") !== -1){
                                            html+=' <span class="btn btn-sm btn-success text-white show_addon_popup" recipe-id="'+recipe['id']+'" group-id="'+group_id+'" restaurant_id="'+$restid+'">Add</span>';
