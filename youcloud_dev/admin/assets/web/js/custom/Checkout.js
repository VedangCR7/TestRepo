var Checkout ={
    ...
    bind_events :function() {
		...
		$('.delivery_payment').on('change',this.onchagepaymentoption);
    ...
    savedeliveryaddress:function(){
		...
		if($('.number').val()<4 && $('.number').val()>14){
			Checkout.displaywarning("Contact number should be 4 to 14 digit in length");
            return false;
		}
        if(edit_id == ''){
            var data={
	...
	onchagepaymentoption:function(){
		if($('input[name=delivery_payment]:checked').val() == 'COD'){
			$('#show_payment_tab').hide();
		}else{
			$('#show_payment_tab').show();
		}
	},

    onchagesupplieroption:function(){
        var delivery_amount =  0;
        var payable_amount =  +Checkout.payable;
        if($('input[name=supply_option]:checked').val() == 'Delivery'){
			$('.payment_count_change').html('3');
            $('#show_address_box').show();
			$('#show_or_not_cod_option').show();
			$('#show_payment_tab').hide();
            // written by victor as part of delivery tracking
            $('.del-free').show();
            delivery_amount = Number($('.delivery_fee').html().trim());
            $('.del-fee').attr('del-fee', delivery_amount.toFixed(2));
            $('.del-fee').html(Checkout.currency_symbol+delivery_amount.toFixed(2));
            
            var total = payable_amount + delivery_amount;

            $('.net-amount').attr('net_total',total.toFixed(2));
            $('.net-amount').html(Checkout.currency_symbol+total.toFixed(2));

            $('.item-total').html(Checkout.currency_symbol+payable_amount);
            $('.item-total').attr('sub_total',payable_amount);

            $('.total_net_amount').val(total.toFixed(2));
            $("#deliver").show();
        }else{
			$('.payment_count_change').html('2');
            $('#show_address_box').hide();
			$('#show_or_not_cod_option').hide();
            $('#show_payment_tab').show();
            // written by victor as part of delivery tracking
            $('.del-fee').attr('del-fee', delivery_amount.toFixed(2));
            $('.del-fee').html(Checkout.currency_symbol+delivery_amount.toFixed(2));
            $("#deliver").hide();
            $('.net-amount').attr('net_total',payable_amount.toFixed(2));
            $('.net-amount').html(Checkout.currency_symbol+payable_amount.toFixed(2));
            $('#deliver').hide();

            $('.item-total').html(Checkout.currency_symbol+payable_amount);
            $('.item-total').attr('sub_total',payable_amount);

            $('.total_net_amount').val(payable_amount.toFixed(2));
        }
    },
    ...
    onPlaceOrder1:function()
	{
		...
		//alert($('input[name=delivery_payment]:checked').val());
		if($('input[name=delivery_payment]:checked').val() =='payment')
		{
			...
			$( ".btn-place-order1" ).prop("disabled", true );
					...
					var data={
						...
						SECRET_KEY : '',
						token : '',
						delivery_payment:$('input[name=delivery_payment]:checked').val()
					};
					/* console.log(data);return false; */
					$('#image-loader').show();
    ...
    loadCartHtml:function(cart){
        ...
        if(cart.length!=0){
            ...
            Checkout.payable = net_amount.toFixed(2);
