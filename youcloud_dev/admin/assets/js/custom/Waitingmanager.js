var Waitingmanager ={
    ...
    bind_events :function() {
        ...
        $('#searchInput').on('keyup',function(){
             ...
             if($(this).val()==""){
                ...
                Waitingmanager.listmanager(data,'fromsearch');
            }else{
                        ...
                        swal({
                            ...
                            if(swalstatus){
                               ...
                               /* self.closest('td').find('.img-upload').attr('data-image-src',imageData);
                                ...
                                $.ajax({
                                    url: Waitingmanager.base_url+"Waiting_manager/update_profile_photo",
                                    type:'POST',
                                    data: $form_data,
                                    processData:false,
                                    contentType:false,
                                    ...
                                    success: function(result){
                                        ...
                                        if (result.status) { 
                                           /* self.closest('td').find('.img-upload').attr('data-image-src',Menugrouplist.base_url+result.path);
                                            ...
                                            if(result.msg){
                                                Waitingmanager.displaywarning(result.msg);
                                            }
                                            else
    ...
    changeStatusManager:function(){
        if($(this).is(':checked')){
            $(this).val("on");
            Waitingmanager.displaysucess("Waiting list manager is Active now");}
        else{
            $(this).val("off");
            Waitingmanager.displaysucess("Waiting list manager is Inactive now");
        }
        ...
        $.ajax({
            url: Waitingmanager.base_url+"Waiting_manager/delete_manager",
            type:'POST',
            data:formData ,
            success: function(result){
               if (result.status) {
    ...
    onEditManager:function(){
        ...
        var formData={
            ...
            $.ajax({
                url: Waitingmanager.base_url+"Waiting_manager/show_perticular_waitinglist_manager",
                type:'POST',
                data:formData ,
                success: function(result){
                        var data={
    ...
    onEditPerticularManager:function(){
        ...
        if ($('#personname').val().length < 2 || $('#personname').val().length > 30) { Waitingmanager.displaywarning("Name field should min 2 to max 30 characters"); }
        else{if ($('#password').val().length < 8 || $('#password').val().length > 30) { Waitingmanager.displaywarning("Password Should be 8 to 30 Characters"); }
        else{
        if ($('#personcontact_number').val().length < 8 || $('#personcontact_number').val().length > 14) { Waitingmanager.displaywarning("Contact Number Should be 8 to 14 digit"); $('.edit_manager').show(); }
        else{
        var self=this;
        var data_id=$(this).attr('data-id');
                var formData={
            ...
            $.ajax({
                url: Waitingmanager.base_url+"Waiting_manager/edit_perticular_waitinglist_manager",
                type:'POST',
                data:formData ,
                success: function(result){
                   if (result.status) {
                        ...
                        $('.edit_manager').hide();
                        Waitingmanager.displaysucess("Information Save successfully");
                       Waitingmanager.listmanager(data);
                   }
                   else{
                        Waitingmanager.displaywarning("Something went wrong please try again");
            ...
            Waitingmanager.displaywarning("Please Fill all the fields");
        }
    },
    ...
    onSaveGroupname:function(){
        ...
        if (!isValid) {
            Waitingmanager.displaywarning("Enter Valid Name");
            return false;
        }
        if($('#name').val()!="" && $('#email').val()!="" && $('#contact_number').val()!=""){
            if ($('#contact_number').val().length < 8 || $('#contact_number').val().length > 14) { Waitingmanager.displaywarning("Contact Number Should be 8 to 14 digit"); }
            else{
                if (/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test($('#email').val()))
                {
                }else{
                    Waitingmanager.displaywarning("Please Enter Valid Email");
                    return false;
                }
            if ($('#name').val().length < 2 || $('#name').val().length > 30) { Waitingmanager.displaywarning("Name field should min 2 to max 30 characters"); }
            else{
            ...
            if ($('#profile_photo').val() != '') {
                $('#image-loader').show();
                $.ajax({
                    url: Waitingmanager.base_url+"Waiting_manager/add_profile_photo",
                    type:'POST',
                    dataType: 'json',
                    data: {
                        name : $('#name').val(),
                    ...
                    success: function(result){
                        if (result.status) {
                            ...
                            if(result.is_email_exist){
                                ...
                                $('#contact_number').val('');
                                //$('#profile_photo').val('');
                                Waitingmanager.displaywarning("Email already exist.");
                            }else{
                                Waitingmanager.displaysucess("User created successfully");
                                var data={
                                    per_page:$('.btn-per-page').attr('selected-per-page'),
                                    page:1
                                }
                            ...
                            $('#profile_photo').val('');
							//$('#profile_photo').val('');
							$('#my_image').attr('src',Waitingmanager.base_url+"assets/images/users/user.png");
                ...
                $('#image-loader').show();
                $.ajax({
                    url: Waitingmanager.base_url+"Waiting_manager/save_waiting_manager",
                    type:'POST',
                    dataType: 'json',
                    data: {
                        name : $('#name').val(),
            ...
            Waitingmanager.displaywarning("Invalid Email");
        }
        }
        }
        }
    ...
    listmanager:function(data,fromevent){
        $.ajax({
            url: Waitingmanager.base_url+"Waiting_manager/list_manager/",
            type:'POST',
            dataType: 'json',
            data: data,
                    ...
                    html+='<tr menu-id="'+managers[i].id+'">\
                            ...
                            if(managers[i].profile_photo==null || managers[i].profile_photo == ''){
                                html+='<td title="Browse">\
                                    ...
                                    <img class="img-upload rounded-circle"  src="'+Waitingmanager.base_url+'assets/images/users/user.png" style="height:50px;width:50px;">\
                                    </td>';
                            }
                            else{
                                 html+='<td title="Browse">\
                                    ...
                                    if(managers[i].profile_photo=="assets/images/users/user.png")
                                        html+='<img class="img-upload rounded-circle" src="'+Waitingmanager.base_url+managers[i].profile_photo+'" style="height:50px;width:50px;"></td>';
                                    else
