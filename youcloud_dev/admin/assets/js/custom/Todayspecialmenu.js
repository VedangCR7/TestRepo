var Recipemenurest ={
	...
	bind_events :function() {
		...
		$('.input-menu-forall').on('click',function(){
			if($(this).is(':checked')){
				...
				$('.input-menu-for-restaurant').each(function(){
					if(!$(this).is(':checked')){
						var id=$(this).val();
						Recipemenurest.uncheck_ids.push(id);
					}
				});
			}
		...
		$('.tbody-recipes-list').on('click','.input-menu-for-restaurant',this.onClickInput);
	},
	...
	saveMenuRestaurant:function(){
		...
		if(ids.length==0 && Recipemenurest.uncheck_ids.length==0){
		...
		swal({
			title: 'Are you sure ?',
			text: " Save this for Todays Special",
			type: 'warning',
			...
			var formData = {
				ids:ids,
				uncheck_ids:Recipemenurest.uncheck_ids,
			...
			$.ajax({
				...
				success: function(result){
				   if (result.status) {
						...
						Recipemenurest.listRecipes(data);
						Recipemenurest.displaysucess("Successfully Saved.");
	...
	listRecipes:function(data,fromevent){
		$('.tbody-recipes-list').html("<h3>Loading Menu...</h3>");
		$.ajax({
