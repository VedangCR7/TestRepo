...
(function (factory) {
            ...
            "<% for(var i = 0; i < numberOfRows; i++){ %>" +
                ...
                "<% var d = j + i * 7; %>" +
                    "<td class='<%= days[d].classes %>'>" +
                        "<div class='col-md-12 header-days'>"+
                            "<div class='col-md-8'>"+
                                "<%= days[d].dayname %> <%= days[d].day %>"+
                            "</div>"+
                            "<div class='col-md-4 text-right'>"+
                                "<a class='btn-add-event <%= days[d].calendar_class %>' href='javascript:;'><i class='fas fa-plus'></i></a>"+
                            "</div>"+
                        "</div>"+
                        "<div class='day-contents mt-7'>"+
                            "<div class='panel-group' id='accordion-<%=d %>' role='tablist' aria-multiselectable='true'>"+
                             "<% var events = days[d].events; for(var s = 0; s < events.length;s++){ %>"+
                                "<div class='panel panel-default mt-2'>"+
                                    "<div class='panel-heading ' role='tab' id='headingOne<%= events[s].date %>-<%=s%>'>"+
                                        "<h4 class='panel-title text-center'>"+
                                            "<a role='button' data-toggle='collapse' data-parent='#accordion-<%=d %>' href='#collapseOne-<%= events[s].date %>-<%=s%>' aria-expanded='true' aria-controls='collapseOne-<%= events[s].date %>-<%=s%>'>"+
                                                 "<%= events[s].title %>"+
                                            "</a>"+
                                        "</h4>"+
                                    "</div>"+
                                    "<div id='collapseOne-<%= events[s].date %>-<%=s%>' class='panel-collapse collapse show' role='tabpanel' aria-labelledby='headingOne<%= events[s].date %>-<%=s%>' >"+
                                        "<div class='panel-body text-left'>"+
                                            "<ul>"+
                                                "<% for(var t = 0; t < events[s].items.length;t++){ %>"+
                                                    "<li class='border-bottom-1 pb-2 pt-1' item-id='<%= events[s].items[t].id %>' event-date='<%= events[s].date %>' event-id='<%= events[s].id %>' event-data='<%= events[s] %>'>"+
                                                        "<a href='javascript:;' class='a-view-item'><%= events[s].items[t].title %></a>"+
                                                        "<a href='javascript:;' class='a-delete-item'><i class='fas fa-times'></i></a>"+
                                                    "</li>"+
                                                "<% } %>" +
                                            "</ul>"+
                                        "</div>"+
    ...
    var defaults = {
        ...
        targets: {
            ...
            addnewEventButton: 'btn-add-event',
    ...
    Clndr.prototype.createDayObject = function (day, monthEvents) {
        ...
        var calendar_class=" calendar-day-" + day.format("YYYY-MM-DD");
        calendar_class += " calendar-dow-" + day.weekday();
    ...
    Clndr.prototype.bindEvents = function () {
        ...
        $container
            ...
            .off(eventName, '.' + targets.addnewEventButton)
        ...
        $container.on(eventName, '.' + targets.addnewEventButton, function (event) {
            var target,
                $currentTarget = $(event.currentTarget);
            if (self.options.clickEvents.onAddDayEvent) {
                target = self.buildTargetObject(event.currentTarget, true);
                self.options.clickEvents.onAddDayEvent.apply(self, [target]);
            }
