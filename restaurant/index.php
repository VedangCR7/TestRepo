<?php 
	...
	$qy = "SELECT * FROM `user` WHERE `business_name` like '$restName'";
...
.pagination ul li{
	padding: 0rem 1rem; color: #000!important; font-weight: 800;
}
...
/* a[data-makeactive]{
  ...
  <body>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark ftco_navbar bg-dark ftco-navbar-light" id="ftco-navbar">
      <div class="container">
        <div class="navbar-brand" onclick="window.history.go(-1); return false;" style="cursor: pointer"><?php echo $restName;?></div>
        
        <!--<div class="collapse navbar-collapse" id="ftco-nav">
          <ul class="navbar-nav ml-auto">
            <div class="navbar-brand"><?php echo $restName;?></div>
          </ul>
        </div>-->
      </div>
    </nav>
    ...
    <div class="showrooms clearfix">
			  ...
			  ?>
				...
				<a class="menu-tab mainmenu logo" data-id="<?php echo $row1['id'];?>" data-makeactive="" data-rid="<?php echo $resto_id; ?>" id="li_<?php echo $row1['id']; ?> v-pills-profile-tab" data-toggle="pill" href="#v-pills-profile" role="tab" aria-controls="v-pills-profile" aria-selected="false">
				<input type="hidden" id="<?php echo $row1['id']; ?>" class="menuGroup_li_<?php echo $i;?>">
				<?php echo ucwords($row1['title']); ?></a>
			  <?php 
					$i++;}} 
 ...
 <span class="arrowright active" id="next"><i class="icon-chevron-right"></i></span>
			...
			<?php
					...
					echo "<center><h3>Restaurant not found.</h3></center>";
				}
				}
				else{
					echo "<center><h3>Restaurant not found.</h3></center>";
...
<!-- End items --> 
	  ...
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  ...
		  <div class="modal-body">
			<ul class="list-unstyled mt-3 mb-4 ul-allgerens-info">
				<li><h5 style="color:#37A445;">Allergens</h5></li>
				<li class="border-bottom-0 p-2 allergens-info-li" style="text-transform:capitalize;">
			...
			<div class="card-body card-padding div-nutrition">
					...
					<span class="foodnai-info-loader" id="nutrition-info-loader">"Loading Information...</span>
				</div>
   ...
   <footer class="ftco-footer ftco-bg-dark ftco-section" style="padding: 11px 0;">
	 ...
	 <?php
		$qry="SELECT * FROM `get_restaurant_count` WHERE `restaurant_id` = $resto_id AND `visited_at`='$today'";
		...
		if($rcnt > 0)
		{
			...
			$updateQry = "UPDATE `get_restaurant_count` SET `no_of_visits`= $newVisitCnt WHERE `restaurant_id`= $resto_id AND `visited_at`='$today'";
			$updateRes = mysqli_query($con, $updateQry);
		}else{
			$insertQry = "INSERT INTO `get_restaurant_count`(`no_of_visits`, `restaurant_id`, `visited_at`) VALUES ('1','$resto_id','$today')";
  ...
  <script src="js/multipleselect/multiple-select.js"></script>
...
<script>
	...
	function get_recipes(a,b,page=1)
	{
		loadedRecieps=[];
		selectedMenuGroup=a;
	...
	//	$(".mainmenu[data-id='"+a+"']").addClass('active');
		//$('[data-id="'+a+'"]').addClass('active');
	...
	function get_allergen_nutrition_info(a,b,c)
	{
		$('#exampleModalLong').modal();
		$.ajax({ 
			type : 'POST',
			url : 'get_allergen_nutrition.php',
			data : {"recipeid":a,"groupid":c},
			success : function(response)
			{
		...
		/* $.ajax({ 
			type : 'POST',
			url : 'get_nutrition_info.php',
			data : {"recipeid":a},
			success : function(response1)
			{
       
				
				$('.div-nutrition').html(response1); 
			},error:function (error) {
				$('.div-nutrition').html("<div class='row'><div class='col-md-12'><p><strong>Allergen-nutrition not found</strong></p></div></div>");
			}
		}); */
		$('.div-nutrition .nutritionals_table').remove();
		$('#nutrition-info-loader').show();
		Reciepe.base_url="https://foodnai.com/admin/";
		Reciepe.recipe_id=a;
		Reciepe.alacalc_recipe_id=b;
		Reciepe.is_nutrition=1;
		Reciepe.init();
		
		setTimeout(function(){
			$('.nutritionals_table .row').removeAttr('style');
			$('#nutrition-info-loader').hide();
			$('.nutritionals_table').show();
		},2000);
		
		$('.recipe-tabs').find('.nutrition .card-body').addClass('active');
	}
	
	function nextRecord(page,resid){
		get_recipes(selectedMenuGroup,resid,page);
	}
...
function loadAlacalcAllergenNutrtion(recipe_id,alacalc_recipe_id){
	...
	$('.menu-tab').on("click", function(e){
       ...
       let reid=$(e.target).attr('data-rid');
       get_recipes(menuid,reid);
  })
...
$(document).on("click", ".open-homeEvents", function () {
	 ...
	 <?php $abc = "<script>document.write(eventId)</script>"?>
});
</script>
  </body>
</html>