  ...
  <body>
    ...
    <div class="showrooms clearfix"> -->
			  ...
			  ?>
				...
				<a class="nav-link py-3 px-4 menu-tab" data-id="<?php echo $row1['id'];?>" id="v-pills-profile-tab" data-toggle="pill" href="#v-pills-profile" role="tab" aria-controls="v-pills-profile" aria-selected="false"><?php echo ucfirst($row1['title']); ?></a>
			  <?php $i++;}} ?>
			  <!-- </div>
			  </div>
...
$('#prev').on('click', function () {
    ...
    last.animate({opacity: '1', width: '250px'});
});
$('#next').on('click', function () {
    var first = $('.logo').first();
    first.animate({opacity: '0', width: '0px'}, function() {
        first.appendTo('.showrooms').css({opacity: '1', width: '250px'});
    });
});
</script>	
