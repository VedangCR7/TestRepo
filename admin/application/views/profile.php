    ...
    <option value="USD">USD</option>
									...
									<div class="admSelectCheck media mt-1 pb-2">
										...
										<select class="form-control" id="restauranttype" name="restauranttype" placeholder="Restaurant Type" required="" selected-value="<?=$user['restauranttype'];?>">
											...
											<option value="vegan" <?php ($user['restauranttype']=="vegan") ? 'selected' : ''; ?>>Vegan</option>
											<option value="keto diet" <?php ($user['restauranttype']=="keto diet") ? 'selected' : ''; ?>>Keto diet</option>
										</select>
...
<!-- 									
						...
						<div class="media mt-1 pb-2">
										...
										<div class="media-body ml-5 mt-1">
											<h6 class="mediafont text-dark mb-1">Cuisines</h6>
											<?php if($user['cuisines'] != ''){ 
											$explodecuisines = explode(',',$user['cuisines']);
											foreach($cuisines as $key=>$value1){
												if (in_array($value1['id'], $explodecuisines)){
											?>
											<div class="col-lg-3 col-md-3 col-sm-3 col-12">
												<input type="checkbox" name="cuisines[]" value="<?=$value1['id']?>" checked>&nbsp&nbsp<?=$value1['cuisines']?>
											</div><?php
												}else{?>
											<div class="col-lg-3 col-md-3 col-sm-3 col-12">
												<input type="checkbox" name="cuisines[]" value="<?=$value1['id']?>">&nbsp&nbsp<?=$value1['cuisines']?>
											</div>
												<?php }
											} } else { 
											foreach($cuisines as $key=>$value1){
						...
						<div class="media mt-1 pb-2">
										...
										<div class="media-body ml-5 mt-1">
											<h6 class="mediafont text-dark mb-1">Average Minimum Price For 2 Person <span >(*)</span></h6>
											<input type="number" class="form-control" name="average_price" value="<?=$user['average_price'];?>">
										</div>
									</div>
						</div>
					</form>
...
<script type="text/javascript">
	...
	$(".a-save-profile").click(function(){
		...
		if($('#form-user [name=average_price]').val()==""){
			displaywarning("Average price can not be blank");
			return false;
		}
