<?php
class Menumaster_model extends My_Model { 
	var $name;
	var $value;
	var $unit;
	var $warning;
    ...
    var $fields=array('name','value','unit','warning','recipe_id','long_desc','declaration_name','img_id');
    public function __construct()
    {
    	$this->load->database();
    }
	
	public function get_menumasterDetails() 
	{
		$condition='';
	...
	{
		$condition='is_delete=0 AND is_active=1 AND logged_user_id='.$resto_id;
	...
	public function add_mastermenu($data)
	{
		$this->db->insert('tbl_recipes_master', $data);
		$insert_id = $this->db->insert_id();
		/* echo $this->db->last_query(); exit; */
		if ($this->db->affected_rows() > 0) 
		{
