...
class Recipes_model extends My_Model {
    ...
    public function list_recipes_typahead(){
    ...
    public function get_recipe($id){
        $this->db->select("r.*,m.title as group_name,ifnull(im.img_path,r.recipe_image) as recipe_image,mm.name as menu_master_name");
        $this->db->from('recipes r');
...
/*
    ...
    public function list_todays_special_userwise($user_id,$main_menu_id,$tablecat_id=""){
        if($tablecat_id!=""){
             $this->db->select("r.*,DATE_FORMAT(recipe_date,'%d %M %Y') as recipe_date,CASE WHEN r.quantity1=".$tablecat_id." THEN r.price1
                 WHEN r.quantity2=".$tablecat_id." THEN r.price2
                 WHEN r.quantity3=".$tablecat_id." THEN r.price3
                else price END as price",FALSE);
            $this->db->from('recipes r');
   ...
   public function make_active($id){    
    ...
    public function list_recipes($page_no,$records_per_page,$group_id="",$main_menu_id="",$searchkey=""){
        ...
        if($recipes_count['cnt']<=0){
             $this->db->select("r.*,m.title as group_name,DATE_FORMAT(recipe_date,'%d %M %Y') as recipe_date,ifnull(im.img_path,r.recipe_image) as recipe_image,im.img_path");
            $this->db->from('recipes r');
            $this->db->join('menu_group m','m.id=r.group_id','left');
            $this->db->join('recipe_images_master im','im.id=r.recipe_image_id','left');
            $this->db->where('r.is_delete',0);
            ...
            if($records_per_page=="all"){
               ...
               /* echo $offset;
                die;*/
                $this->db->select("r.*,m.title as group_name,DATE_FORMAT(recipe_date,'%d %M %Y') as recipe_date,ifnull(im.img_path,r.recipe_image) as recipe_image");
                $this->db->from('recipes r');
                $this->db->join('menu_group m','m.id=r.group_id','left');
                $this->db->join('recipe_images_master im','im.id=r.recipe_image_id','left');
                $this->db->where('r.is_delete',0);
