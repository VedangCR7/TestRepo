<?php
class Restaurant_manager_model extends My_Model {
    var $profile_photo;
    var $name;
    var $email;
    var $contact_number;
    ...
    public function list_manager($page_no,$records_per_page,$searchkey=""){
        $restaurant_count=$this->db->select('count(*) as cnt')->from('user')->where('usertype','Restaurant manager')->where('upline_id',$_SESSION['user_id'])->get()->row_array();
      
        if($restaurant_count['cnt']<=0){
             $this->db->select('*');
            $this->db->from('user');
            $this->db->where('usertype','Restaurant manager');
            $this->db->where('upline_id',$_SESSION['user_id']);
    ...
    public function manager_table($page_no,$records_per_page,$searchkey=""){
        $restaurant_table_count1=$this->db->select('count(DISTINCT(manager_id)) as cnt')->from('assign_table_for_manager')->where('restaurant_id',$_SESSION['user_id'])->get()->row_array();
        
        
        //print_r($restaurant_table_count);exit();
        if($restaurant_table_count1['cnt']<=0){
            $this->db->select('at.*,u.name,u.id as user_id,u.contact_number');
            ...
            return  array(
                'manager'=>$result,
                'total_count'=>$restaurant_table_count1['cnt'],
                'total_pages'=>1,
                'page_no'=>1,
                'page_total_text'=>"0 - ".$restaurant_table_count1['cnt']
            ); 
        }else{
            if($records_per_page=="all"){
                $this->db->select('at.*,u.name,u.id as user_id,u.contact_number');
                ...
                return  array(
                    ...
                    'total_pages'=>1,
                    'page_no'=>1,
                    'page_total_text'=>"1 - ".$restaurant_table_count1['cnt']
                ); 
            }else{
               ...
               /*  echo $this->db->last_query();
                ...
                $total_pages = ceil($restaurant_table_count1['cnt'] / $records_per_page);

                if($restaurant_table_count1['cnt']<$records_per_page)
                    $to_page=$restaurant_table_count1['cnt'];
                else
                    $to_page=$records_per_page;
                return  array(
                    'manager'=>$result,
    ...
    public function waiter_table($page_no,$records_per_page,$searchkey=""){
        $restaurant_table_count=$this->db->select('count(DISTINCT(waiter_id)) as cnt')->from('assign_table_for_waiter')->where('restaurant_id',$_SESSION['user_id'])->get()->row_array();
        //print_r($restaurant_table_count);exit();
        if($restaurant_table_count['cnt']<=0){
            $this->db->select('at.*,u.name,u.id as user_id,u.contact_number');
            $this->db->from('assign_table_for_waiter as at');
            $this->db->join('user as u', 'u.id = at.waiter_id');
            $this->db->where('at.restaurant_id',$_SESSION['user_id']);
            $this->db->group_by('at.waiter_id');
            $query = $this->db->get();
            $result = $query->result_array();
