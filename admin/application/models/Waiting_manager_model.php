...
class Waiting_manager_model extends My_Model {
    ...
    public function list_restaurants($page_no,$records_per_page,$restaurant_status,$searchkey=""){
        $restaurant_count=$this->db->select('count(*) as cnt')->from('user')->where('usertype','Restaurant')->where('step_completed',2)->where('step_one_status!=',NULL)->or_where('step_two_status!=',NULL)->get()->row_array();
      
        if($restaurant_count['cnt']<=0){
             $this->db->select('*');
            $this->db->from('user');
            $this->db->where('usertype','Restaurant');
			$this->db->where('step_completed',2);
			$this->db->where('step_one_status!=',NULL);
			$this->db->or_where('step_two_status!=',NULL);
	...
	public function list_restaurants1($page_no,$records_per_page,$restaurant_status,$searchkey=""){
        $restaurant_count=$this->db->select('count(*) as cnt')->from('user')->where('usertype','Restaurant')->where('step_completed',2)->where('step_one_status',NULL)->where('step_two_status',NULL)->get()->row_array();
        ...
        if($restaurant_count['cnt']<=0){
			...
			$this->db->where('step_completed',2);
			$this->db->where('step_one_status',NULL);
			$this->db->where('step_two_status',NULL);
            if($restaurant_status!="")
                $this->db->where('is_active',$restaurant_status);
            $this->db->order_by("id DESC");
            $query = $this->db->get();
