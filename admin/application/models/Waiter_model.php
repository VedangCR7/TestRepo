<?php
class Waiter_model extends My_Model {
    var $profile_photo;
    var $name;
    var $email;
    var $contact_number;
    ...
    public function list_manager($page_no,$records_per_page,$searchkey=""){
        $restaurant_count=$this->db->select('count(*) as cnt')->from('user')->where('usertype','Waiter')->where('upline_id',$_SESSION['user_id'])->get()->row_array();
      
        if($restaurant_count['cnt']<=0){
             $this->db->select('*');
            $this->db->from('user');
            $this->db->where('usertype','Waiter');
