...
class Restaurant_manager_order_model extends My_Model {
    ...
    public function __construct()
    {
      ...
      // Victor code starts here 
    // check if an order has been assigned to a delivery
    public function getOrderDeliverer($order_id){
        $this->db->select("*");
									
        $this->db->from('assign_delivery_for_order a');
        $this->db->where('a.order_id', $order_id);
		$this->db->where('a.is_accept', 0);
		$this->db->order_by('a.id', 'DESC');
											
        $query = $this->db->get()->row_array();
        $user_id = $query['delieverer_id'];

        if ($query) {
            $this->db->select('*');
            $this->db->from('user');
            $this->db->where('id', $user_id);
            $query['deliverer_name'] = $this->db->get()->row_array();
            return $query;
        }
    }
