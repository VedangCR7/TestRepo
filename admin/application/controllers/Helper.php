...
class Helper extends MY_Controller {
	...
	public function list_manager(){
		if(isset($_POST['searchkey']))
			$manager=$this->Helper_model->list_manager($_POST['page'],$_POST['per_page'],$_POST['searchkey']);
		else
		$manager=$this->Helper_model->list_manager($_POST['page'],$_POST['per_page']);
	...
	public function add_profile_photo(){
    	if(!empty($_POST)){
    		$user_details=$this->Helper_model->check_user($_POST['email']);
	...
	public function save_restaurant_manager(){
		...
		if (!empty($user_details)) {
			...
			$_POST['is_active'] = 1;
			$_POST['usertype'] = 'Helper';
			$_POST['upline_id'] = $_SESSION['user_id'];
			if($this->Helper_model->add_user($_POST)){
			// 	ob_start();
