...
class Table extends MY_Controller {
	...
	public  function create_category()
	{
		...
		$data['result'] = $this->Table_model->get_tableCatDetails($_SESSION['user_id']);
	...
	public  function create_tbl()
	{
		...
		$data['allTblCat'] = $this->Table_model->get_tblCat($_SESSION['user_id']);
		
		$data['result'] = $this->Table_model->get_tableDetails($_SESSION['user_id']);
	...
	public function Add()
	{
		...
		$new_tblNm1 = $this->input->post('new_tblNm'); 
		$new_tblNm2 = $this->input->post('new_tblNm1'); 
		$new_tblNm = $new_tblNm1."".$new_tblNm2; 
		...
		$insert_id = $this->Table_model->add_tableDetails($data1);
		...
		if($insert_id > 0)
		{
			...
			$tblCnt = $this->Table_model->get_tblCount($_SESSION['user_id'],$tblCatId);
			$newTblCnt = ($tblCnt) + 1;
	...
	public function getTblCnt()
	{
		...
		$result = $this->Table_model->get_tblCount($tblCatId); 
	...
	{
		$tbleCatTtl = ucwords($this->input->post('tblCatTtl')); 
		$tblCatTtl = filter_var($tbleCatTtl, FILTER_SANITIZE_STRING);
		$chkbox = ucwords($this->input->post('chkbox')); 
		...
		$insert_id = $this->Table_model->add_tableCategory($data1);
		
		if($insert_id > 0)
		{
			if($chkbox=="1")
			{
				$tname = strtoupper(substr($tblCatTtl, 0, 2));
				
				$data1 = array(	
					'table_category_id' => $insert_id,
					/* 'title' => $tname.'-01', */
					'title' => $tblCatTtl,
					'logged_user_id' => $restoId
				);
				
				$insert_id1 = $this->Table_model->add_tableDetails($data1);
			}
			
			$this->session->set_flashdata('msg', "Table category created successfully.");
			redirect('Table/create_category');
		}
		else
		{
	...
	public function CheckTblName()
	{
		...
		$result = $this->Table_model->check_createdTblNm($newtblname,$restoId);
		if($result!=0)
		{
			echo 'failed';
		}
	...
	public function CheckTblCategory()
	{
		$newtblcategory = addslashes($this->input->post('newtblcategory'));
		$restoId = $_SESSION['user_id']; 
		
		$result = $this->Table_model->check_createdTblCatNm($newtblcategory,$restoId);
		if($result!=0)
		{
			echo 'failed';
		}
	...
	{
		$this->Table_model->delete_tbl($clientId); 
		$this->session->set_flashdata('msg', "Table deleted successfully.");
		redirect('Table/create_tbl', 'refresh');
	...
	{
		/* $this->Table_model->delete_tblCat($clientId); 
		$this->session->set_flashdata('msg', "Table Category deleted successfully.");
		redirect('Table/create_category', 'refresh'); */
	...
	{
		$result = $this->Table_model->getTableCatName($tcatId);
		echo $result; exit;
	}	
