...
class Assistant_chef extends MY_Controller {
	...
	public function __construct() {
		...
		$this->load->model('assistant_chef_model');
	...
	public function assistant_chef() {
		
		$data['restaurantsidebarshow'] =$this->get_menu_of_restaurant();
		$this->load->view('assistant_chef',$data);
	}

	public function list_manager(){
		if(isset($_POST['searchkey']))
			$manager=$this->assistant_chef_model->list_manager($_POST['page'],$_POST['per_page'],$_POST['searchkey']);
		else
		$manager=$this->assistant_chef_model->list_manager($_POST['page'],$_POST['per_page']);
	...
	public function add_profile_photo(){
    	if(!empty($_POST)){
    		$user_details=$this->assistant_chef_model->check_user($_POST['email']);
			if (!empty($user_details)) {
	        	...
	        	if(isset($_POST['image'])){
					...
					if($image_url!=""){
						$m=$this->assistant_chef_model;
						$m->name = $_POST['name'];
						$m->email = $_POST['email'];
						$m->password = $this->randomPassword();
						$m->is_active = 1;
						$m->usertype = 'Assistant Chef';
	...
	public function save_restaurant_manager(){
		...
		if (!empty($user_details)) {
			...
			$_POST['is_active'] = 1;
			$_POST['usertype'] = 'Assistant Chef';
			$_POST['upline_id'] = $_SESSION['user_id'];
			if($this->assistant_chef_model->add_user($_POST)){
			// 	ob_start();
