...
class Admin extends MY_Controller {
	...
	public function all_Cuisines(){
		$data['cuisines'] = $this->waiting_manager_model->select_where_order('cuisines','id','DESC',['is_delete'=>0]);
		$this->load->view('all_cuisines',$data);
	}
	
	public function add_cuisines(){
		$data['cuisines'] = $this->waiting_manager_model->insert_any_query('cuisines',['cuisines'=>$_POST['cuisines'],'is_active'=>1,'is_delete'=>0]);
		$this->json_output(['status'=>true]);
	}
	
	public function delete_cuisines(){
		$this->waiting_manager_model->updateactive_inactive('cuisines',['id'=>$_POST['id']],['is_delete'=>1]);
		$this->json_output(['status'=>true]);
	}
	
	public function cuisine_active_inactive(){
		if($_POST['is_active']=="on"){
			$this->Waiting_manager_model->updateactive_inactive('cuisines',['id'=>$_POST['id']],['is_active'=>1]);
		}			
		else{
			$this->Waiting_manager_model->updateactive_inactive('cuisines',['id'=>$_POST['id']],['is_active'=>0]);
		}
		$this->json_output(['status'=>true]);
		
	}
	
	public function onboarding_step1(){
		$this->load->view('admin_onboarding_step1');
	}
	
	public function admin_onboarding_step2(){
		$this->load->view('admin_onboarding_step2');
	}
	...
	public function all_restaurants1(){
		$sql="SELECT count(id) as total_restaurant FROM user WHERE usertype='Restaurant'";
		$data['total_restaurants']= $this->Waiting_manager_model->query($sql)[0];
		$sql1="SELECT count(id) as active_restaurant FROM user WHERE usertype='Restaurant' AND is_active =1";
		$data['active_restaurants']= $this->Waiting_manager_model->query($sql1)[0];
		$sql2="SELECT count(id) as inactive_restaurant FROM user WHERE usertype='Restaurant' AND is_active =0";
		$data['inactive_restaurants']= $this->Waiting_manager_model->query($sql2)[0];
		$this->load->view('admin_all_restaurants1',$data);
	}
	...
	public function list_restaurants1(){
		if(isset($_POST['searchkey']))
			$manager=$this->Waiting_manager_model->list_restaurants1($_POST['page'],$_POST['per_page'],$_POST['restaurant_status'],$_POST['searchkey']);
		else
		$manager=$this->Waiting_manager_model->list_restaurants1($_POST['page'],$_POST['per_page'],$_POST['restaurant_status']);
		$this->json_output($manager);
	}

	public function delete_restaurant(){
		if($_POST['is_active']=="on"){
			$check = $this->Waiting_manager_model->select_where('user',['id'=>$_POST['id']]);
			if($check[0]['is_new'] == 1){
				$this->Waiting_manager_model->updateactive_inactive('user',['id'=>$_POST['id']],['payment_end_date'=>date('Y-m-d')]);	
			}
	...
	public function delete_category_prices(){
		if($_POST['is_active']=="on"){
			$this->Waiting_manager_model->updateactive_inactive('user',['id'=>$_POST['id']],['is_category_prices'=>1]);
		}			
		else{
		$this->Waiting_manager_model->updateactive_inactive('user',['id'=>$_POST['id']],['is_category_prices'=>1]);}
		$this->json_output(array('status'=>true));
