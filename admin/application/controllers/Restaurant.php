...
class Restaurant extends MY_Controller {
	...
	public  function dashboard($tblType="")
	{
		...
		$sql7 = "SELECT o.id as order_id_for_new_order,o.order_no,o.supply_option,o.net_total,o.status,o.created_at,c.name,td.title,tor.order_type,tor.table_id, tor.id,o.invoice_id,o.is_invoiced,o.order_by,u.name as order_by_name FROM `orders` as o 
		LEFT JOIN table_orders as tor on tor.id=o.table_orders_id 
		LEFT JOIN customer as c on c.id = o.customer_id 
		LEFT JOIN table_details as td on td.id = o.table_id
		...
		WHERE o.rest_id = ".$_SESSION['user_id']." ORDER BY o.id DESC LIMIT 6";
		$data['five_order'] = $this->waiting_manager_model->query($sql7);
		$data['delivery_fee'] = $this->waiting_manager_model->select_where('user',['id'=>$_SESSION['user_id']]);
		$data['recently_added']=$this->recipes_model->list_recently_added_recipes();
		$data['visited_recipes']=$this->recipes_model->list_most_visited_recipes();
		$data['restaurantsidebarshow'] =$this->get_menu_of_restaurant();
	...
	public function monthly_restaurant()
	{
		$data['chartdata'] = [];
		...
		{ 
			...
			$sql = "SELECT ifnull(count(id),0) as custcount FROM user WHERE date >= '".$from_date."' AND date <= '".$last_date."'";
			$chartdata = $this->waiting_manager_model->query($sql);
			$data['chartdata'][$k] = round($chartdata[0]['custcount']);
			array_push($data['chartdata'],$data['chartdata'][$k]);
		}
	...
	public function monthly_order()
	{
		...
		{ 
			...
			$sql = "SELECT ifnull(count(id),0) as custcount FROM orders WHERE created_at >= '".$from_date."' AND created_at <= '".$last_date."'";
			$chartdata = $this->waiting_manager_model->query($sql);
			$data['chartdata'][$k] = round($chartdata[0]['custcount']);
			array_push($data['chartdata'],$data['chartdata'][$k]);
		}
	...
	{
		if(isset($_POST['searchkey']))
			$orders=$this->order_model->list_tablwise_orders_website($_POST['page'],$_POST['per_page'],$_POST['order_date'],$_POST['status'],$_POST['searchkey']);
		else
			$orders=$this->order_model->list_tablwise_orders_website($_POST['page'],$_POST['per_page'],$_POST['order_date'],$_POST['status']);
	...
	public function onlineorder($table_id,$order_id="",$invoice_id="",$from_page="")
	{
		...
		$this->customer_model->updateactive_inactive('orders',['table_orders_id'=>$order_id],['viewed'=>1]);
	...
	public function assignDeliveryOrder($order_id, $user_id="", $table_id=""){
		//echo $order_id;
		$upline_id = $_SESSION['user_id'];
		//$get_perticular_order = $this->waiting_manager_model->select_where('orders',['table_orders_id'=>$order_id]);
		//$order_id=$get_perticular_order[0]['id'];
	...
	public function assign_table_to_manager(){
		$check = $this->waiting_manager_model->select_where('user',['id'=>$_POST['restaurant_manager_id']]);
		...
		$this->json_output(['status'=>true]);
			...
			$this->json_output(['status'=>false,'msg'=>'Restaurant manager is inactive']);
		}
	}
