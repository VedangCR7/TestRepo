...
class Newmaster extends MY_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->is_loggedin();
		$this->load->model('newmaster_model');
	...
	{
		$data=array();
		$this->load->view('newmaster_list',$data);
	}
	
	public function save_menu(){
		echo ($_POST['is_edit_group']);
		die;
		if($_POST['is_edit_group']=='edit'){
			/* $main_menu_id=ucfirst(strtolower($_POST['main_menu_id'])); */
			$m=$this->newmaster_model;
			$m->name=ucfirst(strtolower($_POST['group_name']));
			$m->long_desc=ucfirst(strtolower($_POST['available_in']));
			$m->declaration_name=ucfirst(strtolower($_POST['main_menu_id']));
			$m->recipe_image='assets/images/users/menu.png';
			...
			if(!empty($group_details)){
				...
				/* $this->newmaster_model->update_recipesthumb($main_menu_id,$_POST['group_id']); */
			}
		}else{
			$m=$this->newmaster_model;
			$m->name=ucfirst(strtolower($_POST['group_name']));
