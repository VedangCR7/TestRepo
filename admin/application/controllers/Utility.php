...
class Utility extends MY_Controller {
	...
	public function utility() {
		...
		$this->load->view('utility',$data);
	}

	public function list_manager(){
		if(isset($_POST['searchkey']))
			$manager=$this->utility_model->list_manager($_POST['page'],$_POST['per_page'],$_POST['searchkey']);
		else
		$manager=$this->utility_model->list_manager($_POST['page'],$_POST['per_page']);
	...
	public function add_profile_photo(){
    	if(!empty($_POST)){
    		$user_details=$this->utility_model->check_user($_POST['email']);
	...
	public function save_restaurant_manager(){
		...
		if (!empty($user_details)) {
			...
			$_POST['is_active'] = 1;
			$_POST['usertype'] = 'Utility';
			$_POST['upline_id'] = $_SESSION['user_id'];
			if($this->utility_model->add_user($_POST)){
			// 	ob_start();
