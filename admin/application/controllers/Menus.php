...
class Menus extends MY_Controller {
	...
	public function get_addon_details()
	{
		...
		{
			$sql="SELECT r.id as recipe_id,r.name as recipe_name,r.price as recipe_price,r.recipe_type,ifnull(d.discount,0)  as discount,d.id as offer_id,d.discount_type
			FROM recipes AS r
		...
		else
		{
			if(!empty($check_menu_id_available) && !empty($check_menu_group))
			{
				$sql="SELECT r.id as recipe_id,am.*,r.name as recipe_name,r.price as recipe_price,r.recipe_type,ifnull(d.discount,0)  as discount,d.id as offer_id,d.discount_type
		...
		foreach($addon_menu as $key=>$value)
		{
			...
			$addon_data[] = array('recipe_id'=>$value['recipe_id'],'recipe_name'=>$value['recipe_name'],'recipe_price'=>$value['recipe_price'],'recipe_type'=>$value['recipe_type'],'discount'=>$value['discount'],'offer_id'=>$value['offer_id'],'discount_type'=>$value['discount_type'],'addon_id'=>$value['id'],'addon_name'=>$value['addon_name'],'is_multiple_menu'=>$value['is_multiple_menu'],'options'=>$addon_options);
		}
