...
*/		$data['usertype']=$_SESSION['usertype'];
		...
		$data['cuisines'] = $this->Waiting_manager_model->select_where('cuisines',['is_delete'=>0,'is_active'=>1]);
	...
	public function update_profile_for_admin(){
		$get_country_code=explode(',',$_POST['forcountry_countrycode']);
		$_POST['forcountry_countrycode']= $get_country_code[0];
		$u = $this->user_model;
		...
		$this->Waiting_manager_model->updateactive_inactive('user',['id'=>$_POST['id']],$_POST);
		
		$this->json_output(array('status'=>true,'msg'=>"profile updated successfully !"));
		
	}

	public function update_profile(){
		$_POST['cuisines'] = implode(',',$_POST['cuisines']);
		$get_country_code=explode(',',$_POST['forcountry_countrycode']);
		$_POST['forcountry_countrycode']= $get_country_code[0];
		$u = $this->user_model;
	 ...
	 die;*/
        if(isset($_POST)){
        	...
        	if(isset($_POST['image'])){
				...
				$image_url='Liveresto/profile/'.$rand_no.'.jpg';
	...
	public function update_restaurant_photo1(){
    	if(isset($_POST)){
        	...
        	if(isset($_POST['image'])){
	        	...
	        	$rand_no=rand(1111111,9999999);
        		/*if(SERVER=="testing")
					$image_url='test/restimg/'.$rand_no.'.jpg';
				else
					$image_url='restimg/'.$rand_no.'.jpg';*/
				$image_url='Liveresto/restimg/'.$rand_no.'.jpg';
	        	$file_path='uploads/restimg/'.$rand_no.'.jpg';
                $img_r = imagecreatefromjpeg($_POST['image']);
				$output=imagejpeg($img_r,$file_path);
				$aws_result=$this->uploadAWSS3($image_url,$file_path);
				//unlink($file_path);
				...
				if($image_url!=""){
					$u=$this->user_model;
		        	if($image_url!="")
		                $u->rest_img_1=CLOUDFRONTURL.$image_url;
		            
		                $u->id=$_SESSION['user_id'];
		                $u->update();
		                $user_id=$_SESSION['user_id'];
					
		           $this->json_output(array('path'=>CLOUDFRONTURL.$image_url,'status'=>true,'msg'=>'Restaurant Photo Upated'));
					return;
				}else{
					$this->json_output(array('status'=>false,'msg'=>'Something went wrong please try again.'));
					return;
    ...
    public function update_restaurant_photo2(){
    	if(isset($_POST)){
        	...
        	if(isset($_POST['image'])){
				...
				if($image_url!=""){
					$u=$this->user_model;
		        	if($image_url!="")
		                $u->rest_img_2=CLOUDFRONTURL.$image_url;
    ...
    public function update_restaurant_photo3(){
    	if(isset($_POST)){
        	...
        	if(isset($_POST['image'])){
				...
				if($image_url!=""){
					$u=$this->user_model;
		        	if($image_url!="")
		                $u->rest_img_3=CLOUDFRONTURL.$image_url;
    ...
    public function update_restaurant_photo4(){
    	if(isset($_POST)){
        	...
        	if(isset($_POST['image'])){
				...
				if($image_url!=""){
					$u=$this->user_model;
		        	if($image_url!="")
		                $u->rest_img_4=CLOUDFRONTURL.$image_url;
    ...
    public function update_restaurant_photo5(){
    	if(isset($_POST)){
        	...
        	if(isset($_POST['image'])){
				...
				if($image_url!=""){
					$u=$this->user_model;
		        	if($image_url!="")
		                $u->rest_img_5=CLOUDFRONTURL.$image_url;
