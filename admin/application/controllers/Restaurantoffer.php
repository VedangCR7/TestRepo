	...
	public function save_offer()
	{
		...
		foreach($_POST['recipe_id'] as $key=>$value){
			...
			if(empty($check_recipe_added)){
				if($_POST['discount_type'] == 'Flat'){
					...
					if($count<=0)
					{
						...
						if($count1 <= 0){
							...
							array_push($check_recipes_price,$data1[0]['name']);
						}
					}
				}else{
					$query="SELECT * from admin_offer WHERE title='".$title."' and recipe_id='".$value."' and discount='".$discount."'";
				...
				$this->json_output(array('status'=>false,'msg'=>"Offer already created for  recipe"));
				exit();
			}
		}
		
		if(!empty($check_recipes_price)){
			$recipe = implode(',',$check_recipes_price);
			$this->json_output(array('status'=>false,'msg'=> $recipe." price is greater than recipe price."));
			exit();
		}
