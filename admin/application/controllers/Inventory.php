...
class Inventory extends MY_Controller {
	...
	public function uploadData(){
		...
		try {
			...
			if($check_empty_file ==0){
				$this->json_output(array('status'=>false,'msg'=>'File records are empty'));
				exit();
			}
	...
	public function save_supplier()
	{
		...
		if(!empty($_POST))
		{
			...
			// else{
				...
				$check_supplier_email_exist = $this->inventory_supplier_model->select_where('inventory_suppliers',['email'=>$_POST['email'],'restaurant_id'=>$_SESSION['user_id']]);
	        	
				if(!empty($check_supplier_email_exist)){
					$this->json_output(array('status'=>false,'msg'=>'Email Already Exist for previous supplier'));
					return;
				}
	...
	public function purchase_invoice_details(){
		...
		if(empty($invoice_details)){
			...
			$data = array('invoice_no'=>$formated_id,'created_at'=>date('Y-m-d H:i:s'),'purchase_id'=>$purchase_id,'supplier_id'=>$purchase_details[0]['supplier_id'],'grand_total'=>$purchase_details[0]['grand_total']);
	...
	public function purchase_order_placeorder(){
		...
		$sql199 = "SELECT * from inventory_product_assign_kitchen order by DESC limit 1";
		$assign_product_last_entries = $this->inventory_product_model->query($sql199);
		if(empty($assign_product_last_entries)){
				$last_id = 1;
				$formated_id=str_pad($last_id, 7, '0', STR_PAD_LEFT);
			}else{
				$last_id = $assign_product_last_entries[0]['id']+1;
				$formated_id=str_pad($last_id, 7, '0', STR_PAD_LEFT);
			}
		$restaurant = $this->inventory_product_model->select_where('user',['id'=>$rest_id])[0];
		//$cart_details=$this->inventory_product_model->get_cart_menu_details();
		//print_r($cart_details);exit();
		$_POST['total_purchase_quantity'] = 0;
		$_POST['total_assign_quantity']=0;
		$_POST['total_remaining_quantity'] = 0;
		foreach ($cart_details as $cart) 
		{
			$sql000 = "SELECT * FROM inventory_product_assign_kitchen_items WHERE product_id=".$cart['product_id']." AND supplier_id=".$_POST['supplier_id']." ORDER BY id DESC LIMIT 1";
			$get_last_item = $this->inventory_product_model->query($sql000);
			$_POST['total_purchase_quantity']=$cart['qty']+$get_last_item[0]['remaining_quantity'];
		}
		...
		$o->total_remaining_quantity=$_POST['total_purchase_quantity'];
		$o->restaurant_id=$rest_id;
		$o->supplier_id=$_POST['supplier_id'];
		$o->date=$_POST['date'];
		$o->assign_no = $formated_id;
		...
		foreach ($cart_details as $cart) 
		{
			...
			$oi->product_id=$cart['product_id'];
			$oi->purchase_quantity=$cart['qty']+$get_last_item[0]['remaining_quantity'];
			$oi->assign_quantity=0;
			$oi->remaining_quantity=$cart['qty']+$get_last_item[0]['remaining_quantity'];
			$oi->supplier_id=$_POST['supplier_id'];
			$oi->restaurant_id=$rest_id;
			$oi->add();
		}
	...
	public function list_product_for_assign_kitchen(){
		/*echo $sql = "SELECT pi.product_id as id , pro.product_name as name,SUM(pi.qty) as qty,ifnull(SUM(ipa.assign_quantity),0) as assign_qty 
		FROM inventory_purchase_item AS pi
		LEFT JOIN inventory_product AS pro on pro.id = pi.product_id 
		LEFT JOIN inventory_purchase AS p on p.id = pi.purchase_id 
		LEFT JOIN inventory_suppliers AS s on s.id = p.supplier_id 
		LEFT JOIN inventory_product_assign_kitchen_items AS ipa on ipa.product_id = pi.product_id 
		WHERE p.supplier_id =".$_GET['supplier_id']." GROUP BY pi.product_id";
		
		$products=$this->inventory_product_model->query($sql);*/
		$sql="SELECT pi.product_id as id , pro.product_name as name,SUM(pi.qty) as qty 
		FROM inventory_purchase_item AS pi
		LEFT JOIN inventory_product AS pro on pro.id = pi.product_id 
		LEFT JOIN inventory_purchase AS p on p.id = pi.purchase_id 
		LEFT JOIN inventory_suppliers AS s on s.id = p.supplier_id
		WHERE p.supplier_id =".$_GET['supplier_id']." GROUP BY pi.product_id";
		$product=$this->Waiting_manager_model->query($sql);
		//print_r($product);
		$product1 = [];
		foreach($product as $key=>$value){
			$sql1 = "SELECT ipa.assign_quantity as assign_qty,ipa.purchase_quantity as purchase_qty,ipa.remaining_quantity
			FROM inventory_product_assign_kitchen_items AS ipa
			WHERE ipa.product_id =".$value['id']." AND ipa.supplier_id=".$_GET['supplier_id']." ORDER BY id DESC LIMIT 1";
			$check_assign_quantity = $this->Waiting_manager_model->query($sql1);
			array_push($product1,['id'=>$value['id'],'name'=>$value['name'],'qty'=>$check_assign_quantity[0]['remaining_quantity']]);
			
		}
		//print_r($products);
		$this->session->unset_userdata('cart_contents');
		$this->json_output($product1);
	}
