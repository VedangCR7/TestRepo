var Signup ={
    ...
    bind_events :function() {
		...
		$(".input-contact").on("keypress keyup",function(){
			if($(this).val() == '0'){
			  $(this).val('');  
			}
		});
        $('.form-singup').on('submit',this.onSubmitCustomerform);
        $('.input-contact').on('change',this.onCheckContact);
    ...
    onCheckContact:function()
	{
        ...
        var data={
            ...
            restaurant_id:$('.input-rest-id').val()
        }
        $('#image-loader').show();
        $.ajax({
    ...
    onSubmitCustomerform:function()
	{
        ...
        var is_website=$.trim($('.is_website').val());
        ...
        $.ajax({
            ...
            success: function(result)
			{
				...
				{ 
                    if(Signup.tableid!="")
                        window.location.href=Signup.base_url+"mainmenu/mainmenu/"+Signup.restid+"/"+Signup.tableid+"/"+is_website;
                    else
