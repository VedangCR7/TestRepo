var Recipeadmin ={
	...
	bind_events :function() {
		...
		$('#searchRecipeInput').on('keyup',function(){
			if($(this).val()==""){
				...
				Recipeadmin.listRecipes(data,'fromsearch');
			}else{
				if($(this).val().length>=3){
					var data={
						...
						page:1,
						group_id:Recipeadmin.group_id,
						search_name:$('#searchRecipeInput').val()
					}
					Recipeadmin.listRecipes(data,'fromsearch');
				}
			}
	...
	saveMenuRestaurant:function(){
		...
		if(ids.length==0){
			Recipeadmin.displaywarning("Please select at least on recipe.");
			return false;
		}*/
		swal({
			title: 'Are you sure ?',
			text: "Add Recipe for Menu for Restaurants",
			type: 'warning',
			...
			$.ajax({
				url: Recipeadmin.base_url+"admin/save_menu_for_restaurant",
				type:'POST',
				data:formData ,
				success: function(result){
				   ...
				   if (result.status) {
						...
						Recipeadmin.displaysucess("Successfully Added.");
						window.location.href=Recipeadmin.base_url+"admin/menufor_restaurant";
				   }
				   else{
						Recipeadmin.displaywarning("Something went wrong please try again");
				   }
				}
			});
	...
	listRecipes:function(data,fromevent){
		$.ajax({
			url: Recipeadmin.base_url+"admin/all_recipes/",
			type:'POST',
			dataType: 'json',
			data: data,
			success: function(response){
				...
				for (i in recipe) {
					...
					html+='<tr>';
						if(recipe[i].isadded_for_restaurant==1)
						...
						html+='<td>\
							<a href="'+Recipeadmin.base_url+'recipes/create/'+recipe[i].id+'" data-id="'+recipe[i].id+'" alacalc-recipe-id="'+recipe[i].alacal_recipe_id+'" style="color:#000;">'+recipe[i].name+'</a>\
						</td>\
					</tr>';
				}
