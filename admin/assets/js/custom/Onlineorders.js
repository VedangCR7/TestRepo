var Onlineorders ={
	...
	bind_events :function() {
		...
		// added by victor
        	$("#deliverer").on("click", this.showDeliverers);
        ...
        var html='<div class="tab-pane active " id="invoice-'+bill_count+'">\
			...
			\
				...
				<tr>\
									...
									<tr>\
										<td class="text-right td-totaldetails show_discount_note" style="display:none;">\
										</td>\
									</tr>\
									<tr>\
    ...
    showOrderDetails:function(result,tab_pane,order_type="")
	{
        ...
        tab_pane.find('.subtotal_html').html('Sub Total : '+Onlineorders.currency_symbol+" "+ result['sub_total']);
		 ...
		 if (result['supply_option'] == 'Delivery') {
            tab_pane.find('.delivery_html').html('Delivery Fee: '+Onlineorders.currency_symbol+" "+ result['delivery_fee']);
			$("#deliverer").show();
        } else {
            tab_pane.find('.delivery_html').hide()
        }
		...
		else
		{
            ...
            //By victorchanged result['sub_total] to result['net_total]*/
        if (result['supply_option'] == "Delivery") {
            nettotal = (parseFloat(nettotal) + parseFloat(result['delivery_fee'])).toFixed(2);
			$("#deliverer").show();
        }
		...
		tab_pane.find('.show_discount_note').hide();
        ...
        else if(result['status']=="Canceled")
		{
           Onlineorders.showOrderviewMode(tab_pane);
			...
			tab_pane.find('.placeorder_kitchen_print').hide();
			$("#deliverer").hide();
			//tab_pane.find('.btn-block-customer').hide();
        }
		...
		else if(result['status']=="Assigned to deliver" || result['status'] == "Reassign Delivery")
		{
            status_color="badge-info";
            /*tab_pane.find('.btn-served').show();*/
		...
		else if(result['status']=="Order picked up" || result['status'] == "Order transit")
		{
            status_color="badge-info";
            /*tab_pane.find('.btn-served').show();*/
		...
		if(result['status'] == "Assigned to deliver" || result['status'] == "Reassign Delivery"){
            tab_pane.find('.placeorder_kitchen_for_view').hide();
            $("#deliverer").show();
        }												   
    },
    ...
    showInvoiceDetails:function(result,tab_pane,order_type)
	{
		...
		//if(result['supply_option'] != 'Collection'){
		//tab_pane.find('.delivery_html').html('Delivery_fee : '+Onlineorders.currency_symbol+" "+ result['delivery_fee']);}			
        tab_pane.find('.distotal_html').html('Discount Total : '+Onlineorders.currency_symbol+" "+ result['disc_total']);
		if(result['supply_option'] != 'Collection'){
			tab_pane.find('.nettotal_html').html('Net Total '+Onlineorders.currency_symbol+" "+ (parseInt(result['net_total'])+parseInt(result['delivery_fee'])));
        }else{
		...
		else
		{
			...
			tab_pane.find('.show_discount_note').show();
			tab_pane.find('.show_discount_note').html('<span style="float:left;">Discount Note : '+result['final_note']+'</span>');
		}
    ...
    getorderdetails:function()
	{
		...
		// code added by victor
				var invoices=response['invoices'];
				var supply_option = '';
				if(orders != ''){
					supply_option = orders[0].supply_option;
					var tab_pan=$(this).closest('.tab-pane');
					
					if (supply_option == "Delivery") {
						Onlineorders.order_id = orders[0].id;		
					}
					var checkItem = localStorage.getItem("confirmed")
					if (supply_option == "Delivery" && checkItem === "true") {
						$('#deliverer').show();
						$('.placeorder_kitchen_for_view').hide();
					}
					else{
						$('#deliverer').hide();
					}
					...
					if(supply_option == "Delivery"){
						$('#show_payment_status').html('Payment Status : '+orders[0].delivery_payment);
						$('#show_payment_status').show();
					}else{
						$('#show_payment_status').hide();
					}
				}else{
					supply_option = invoices[0].supply_option;
					var tab_pan=$(this).closest('.tab-pane');
					...
					localStorage.removeItem("confirmed");
					if(supply_option == "Delivery"){
						$('#show_payment_status').html('Payment Status : '+invoices[0].delivery_payment);
						$('#show_payment_status').show();
					}else{
						$('#show_payment_status').hide();
					}
				...
				for(i in orders)
				{
                    if(i==0)
					{
						...
						orders[i]['delivery_fee'] = response['restaurant'].delivery_fee;
				...
				var delivery_fee = response['restaurant'].delivery_fee;
   ...
   displaywarning:function(msg)
	...
	quantity_plus:function()
	{
	  ...
	  // Code added By Victor starts here 
    showDeliverers: function () {
        var url = $(location).attr('href').split('/');
        var order_id = url[url.length - 1]
        var url = $(location).attr('href').split('/');
        let check = "false"
		var order_id = Onlineorders.order_id
		//alert(order_id);
        debugger;
        $.ajax({
            url: Onlineorders.base_url + `restaurant/assignDeliveryOrder/${order_id}`,
            method:"GET",
            success: function (response) {
				console.log(response['res']);
                if (response['res']) {
                    $("#modal_message").removeAttr("hidden");
                    $("#modal_message").html(`This order is assigned to "${response['res']['deliverer_name'].name}"`);
                    check = "true";
                    console.log(check);
                } 
                    $.ajax({
                        url: Onlineorders.base_url+"delivery/list_deliverers/",
                        method: "GET",
                        success: function(response){
                            // console.log(response['']);
                            var html = `
                            ...
                            data.forEach((deliverer, index) => {
                                html += `
                                    <tr>
                                        <th scope="row">${index+1}</th>
                                        <td>${deliverer["name"]}</td>
                                        <td> 
                                            <button class="btn btn-success btn-sm" id= ${deliverer['id']} onclick="Onlineorders.assignDeliverer(${deliverer['id']})">Assign</button>
                                        </td>
                                    </tr>
                                `
                            });
                            html += `</tbody></table>`
                            $("#deliverer_list").html(html);
                        }
                    });
            }
        });
    ...
    assignDeliverer: function(id){
        var url = $(location).attr('href').split('/');
        var last = url[url.length -1]
        var order_id = Onlineorders.order_id
        if (last == "tableorders") {
            table_id = url[url.length- 4]
        }
        else{
            table_id = last
        }
        ...
        $.ajax({
            url: Onlineorders.base_url+`restaurant/assignDeliveryOrder/${order_id}/${id}/${table_id}`,
            method: "GET",
            success: function(response){
				console.log(response);
                $("#modal_message").removeAttr("hidden");
                if (response['res']) {
                    $("#modal_message").html(`This order is assigned to "${response['res']['deliverer_name'].name}"`);
                     ...
                     $("#modal_message").text(response['data'])
                }else{
                    $("#modal_message").text(response['error']);
                }
                setTimeout(() => {
                    $("#modal_message").attr("hidden", true);
                }, 3000);
            }
        })
    },
