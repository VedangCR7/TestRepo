var Waiter ={
    ...
    bind_events :function() {
        ...
        $('#searchInput').on('keyup',function(){
             ...
             if($(this).val()==""){
                ...
                Waiter.listmanager(data,'fromsearch');
            }else{
    ...
    assign_table_to_manager:function(){
        ...
        console.log(array);
        $.ajax({
            url: Waiter.base_url+"assistant_chef/assign_table_to_manager",
            type:'POST',
            data:{restaurant_waiter_id:restaurant_waiter_id,table:array},
            success: function(result){
                if(result.status){
                    $('#assigntablemodal').modal('hide');
                    Waiter.displaysucess("Assigned table to waiter");
                }else{
                    Waiter.displaywarning('Something went wrong');
                    return false;
                }
                var data={
                    per_page:$('.btn-per-page').attr('selected-per-page'),
    ...
    onassigntable:function(){
        ...
        $.ajax({
            url: Waiter.base_url+"assistant_chef/show_table_rest_manager",
            type:'POST',
                ...
                // }else{
                    ...
                    for(var i = 0;i<result.table.length;i++){
                        html +='<div class="col-lg-3 col-md-3 col-sm-12 col-12">';
                            ...
                            html +='</div>';
                            $('#assign_table').attr('disabled',false);
                    }
                }
                else{
    ...
    onImageload:function(event){
        ...
        var ext = $(this).val().split('.').pop().toLowerCase();
            if($.inArray(ext, ['jpg','jpeg','png']) == -1) {
                Waiter.displaywarning('invalid extension!');
                return false;
            }
            ...
            if (target.files && target.files[0]) {
                ...
                // Here you can ask your users to load correct file
                    $('#image-loader').hide();
                    Waiter.displaywarning("File size is too big. please select the file less than 1MB.");
                    return false;
                }
            }
            ...
            if (window.File && window.FileList && window.FileReader) {
                ...
                Waiter.displaywarning("Your browser does not support File API");  
                $("#cropbox").attr('src','');
                $('#image-loader').hide();
            }  
    },
                        ...
                        swal({
                            ...
                            if(swalinput){
                               ...
                               /* self.closest('td').find('.img-upload').attr('data-image-src',imageData);
                                ...
                                $.ajax({
                                    url: Waiter.base_url+"assistant_chef/update_profile_photo",
                                    type:'POST',
                                    data: $form_data,
                                    processData:false,
                                    contentType:false,
                                    ...
                                    success: function(result){
                                        ...
                                        if (result.status) { 
                                           /* self.closest('td').find('.img-upload').attr('data-image-src',Menugrouplist.base_url+result.path);
                                            ...
                                            if(result.msg){
                                                Waiter.displaywarning(result.msg);
                                            }
                                            else
                                                Waiter.displaywarning("Something went wrong please try again");
                                        }
                                        $('#image-loader').hide();
                                        var data={
                                            per_page:$('.btn-per-page').attr('selected-per-page'),
    ...
    changeStatusManager:function(){
        if($(this).is(':checked')){
            $(this).val("on");
            Waiter.displaysucess("Waiter is Active now");}
        else{
            $(this).val("off");
            Waiter.displaysucess("Waiter is Inactive now");}
        ...
        $.ajax({
            url: Waiter.base_url+"assistant_chef/delete_manager",
            type:'POST',
            data:formData ,
            success: function(result){
               if (result.status) {
    ...
    onEditManager:function(){
        ...
        var formData={
            ...
            $.ajax({
                url: Waiter.base_url+"Waiting_manager/show_perticular_waitinglist_manager",
                type:'POST',
                data:formData ,
                success: function(result){
                        var data={
                            ...
                            <form class="form-recipe-edit" method="post" action="javascript:;">\
                                <div class="row">\
                                    <div class="col-lg-3 col-md-3 col-sm-12 col-12" style="margin-top:10px;">\
                                        ...
                                        <input type="text" name="personname" id="personname" value="'+result.name+'" class="form-control" placeholder="Enter Name">\
                                    </div>\
                                    <div class="col-lg-3 col-md-3 col-sm-12 col-12" style="margin-top:10px;">\
    ...
    onEditPerticularManager:function(){
        ...
        if (!isValid) {
            Waiter.displaywarning("Enter Valid Name");
            return false;
        }
        if ($('#personname').val() != '' && $('#personcontact_number').val() != '' && $('#password').val() != '') { 
        if ($('#personname').val().length < 2 || $('#personname').val().length > 30) { Waiter.displaywarning("Name field should min 2 to max 30 characters"); }
        else{if ($('#password').val().length < 8 || $('#password').val().length > 30) { Waiter.displaywarning("Password Should be 8 to 30 Characters"); }
        else{
        if ($('#personcontact_number').val().length < 8 || $('#personcontact_number').val().length > 14) { Waiter.displaywarning("Contact Number Should be 8 to 14 digit"); $('.edit_manager').show(); }
        else{
        var self=this;
        var data_id=$(this).attr('data-id');
        //alert(data_id);
        var formData={
            ...
            $.ajax({
                url: Waiter.base_url+"Waiting_manager/edit_perticular_waitinglist_manager",
                type:'POST',
                data:formData,
                success: function(result){
                   if (result.status) {
                    Waiter.displaysucess("Information Save successfully");
                        var data={
                            per_page:$('.btn-per-page').attr('selected-per-page'),
                            page:1
                        }
            ...
            Waiter.displaywarning("Please Fill all the fields");
        }
    ...
    onDeleteManager:function(){
        ...
        swal({
            ...
            $.ajax({
                url: Waiter.base_url+"assistant_chef/delete_perticular_restaurant_manager",
                type:'POST',
                data:formData ,
                success: function(result){
                   if (result.status) {
                    Waiter.displaysucess("Delete successfully");
                        var data={
                            per_page:$('.btn-per-page').attr('selected-per-page'),
                            page:1
                        }
    ...
    onSaveGroupname:function(){
        ...
        if($('#name').val()!="" && $('#email').val()!="" && $('#contact_number').val()!=""){
            if (/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test($('#email').val()))
            {
                ...
                Waiter.displaywarning("Please Enter Valid Email");
                return false;
            }
            if ($('#name').val().length < 2 || $('#name').val().length > 30) { Waiter.displaywarning("Name field should min 2 to max 30 characters"); }
            else{
            if ($('#contact_number').val().length < 8 || $('#contact_number').val().length > 14) { Waiter.displaywarning("Contact Number Should be 8 to 14 digit"); }
            ...
            if ($('#profile_photo').val() != '') {
                $('#image-loader').show();
                $.ajax({
                    url: Waiter.base_url+"assistant_chef/add_profile_photo",
                    type:'POST',
                    dataType: 'json',
                    data: {
                        name : $('#name').val(),
                    ...
                    success: function(result){
                        ...
                        if (result.status) {
                            if(result.is_email_exist){
                                Waiter.displaywarning("Email already exist.");
                            }else{
                                Waiter.displaysucess("Waiter created successfully");
                                var data={
                                    per_page:$('.btn-per-page').attr('selected-per-page'),
                                    page:1
                                }
							...
							$('#my_image').attr('src',Waiter.base_url+'assets/images/users/user.png');
                ...
                $('#image-loader').show();
                $.ajax({
                    url: Waiter.base_url+"assistant_chef/save_restaurant_manager",
                    type:'POST',
                    dataType: 'json',
                    data: {
                        name : $('#name').val(),
            ...
            Waiter.displaywarning("Invalid Email");
        }
        }
        }
        }
    ...
    listmanager:function(data,fromevent){
        $.ajax({
            url: Waiter.base_url+"assistant_chef/list_manager/",
            type:'POST',
            dataType: 'json',
            data: data,
            success: function(response){
                ...
                for (i in managers) {
                    ...
                    html+='<tr menu-id="'+managers[i].id+'">\
                            <td>'+ j +'</td>';
                            if(managers[i].profile_photo==null || managers[i].profile_photo == ''){
                                html+='<td title="Browse">\
                                    ...
                                    <img class="img-upload rounded-circle"  src="'+Waiter.base_url+'assets/images/users/user.png" style="height:50px;width:50px;">\
                                    </td>';
                            }
                            else{
                                  html+='<td title="Browse">\
                                    ...
                                    if(managers[i].profile_photo=="assets/images/users/user.png")
                                        html+='<img class="img-upload rounded-circle" src="'+Waiter.base_url+managers[i].profile_photo+'" style="height:50px;width:50px;"></td>';
                                    else
