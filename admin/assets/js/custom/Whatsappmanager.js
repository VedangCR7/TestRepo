var Whatsappmanager ={
    ...
    bind_events :function() {
        ...
        $('.a-recipe-perpage').on('click',function(){
            ...
            Whatsappmanager.listmanager(data);
        });
        ...
        $('#searchInput').on('keyup',function(){
             ...
             if($(this).val()==""){
                ...
                Whatsappmanager.listmanager(data,'fromsearch');
            }else{
    ...
    onImageload:function(event){
        ...
        var ext = $(this).val().split('.').pop().toLowerCase();
            if($.inArray(ext, ['jpg','jpeg','png']) == -1) {
                Whatsappmanager.displaywarning('invalid extension!');
                return false;
            }
            ...
            if (target.files && target.files[0]) {
                ...
                // Here you can ask your users to load correct file
                    $('#image-loader').hide();
                    Whatsappmanager.displaywarning("File size is too big. please select the file less than 1MB.");
                    return false;
                }
            }
            ...
            if (window.File && window.FileList && window.FileReader) {
                ...
                Whatsappmanager.displaywarning("Your browser does not support File API");  
                $("#cropbox").attr('src','');
                $('#image-loader').hide();
            }  
    },
                        ...
                        swal({
                            ...
                            if(swalstatus){
                               ...
                               /* self.closest('td').find('.img-upload').attr('data-image-src',imageData);
                                ...
                                $.ajax({
                                    url: Whatsappmanager.base_url+"Whatsapp_manager/update_profile_photo",
                                    type:'POST',
                                    data: $form_data,
                                    processData:false,
                                    contentType:false,
                                    ...
                                    success: function(result){
                                        ...
                                        if (result.status) { 
                                           /* self.closest('td').find('.img-upload').attr('data-image-src',Menugrouplist.base_url+result.path);
                                            ...
                                            if(result.msg){
                                                Whatsappmanager.displaywarning(result.msg);
                                            }
                                            else
                                                Whatsappmanager.displaywarning("Something went wrong please try again");
                                        }
                                        $('#image-loader').hide();
                                        var data={
                                            per_page:$('.btn-per-page').attr('selected-per-page'),
    ...
    changeStatusManager:function(){
        if($(this).is(':checked')){
            $(this).val("on");
            Whatsappmanager.displaysucess("Whatsapp manager is Active now");}
        else{
            $(this).val("off");
            Whatsappmanager.displaysucess("Whatsapp manager is Inactive now");
        }
        ...
        $.ajax({
            url: Whatsappmanager.base_url+"Whatsapp_manager/delete_manager",
            type:'POST',
            data:formData ,
            success: function(result){
               if (result.status) {
                //Whatsappmanager.displaysucess("Status Changed successfully");
                    var data={
                        per_page:$('.btn-per-page').attr('selected-per-page'),
                        page:1
                    }
    ...
    onEditManager:function(){
        ...
        var formData={
            ...
            $.ajax({
                url: Whatsappmanager.base_url+"Whatsapp_manager/show_perticular_waitinglist_manager",
                type:'POST',
                data:formData ,
                success: function(result){
                        var data={
    ...
    onEditPerticularManager:function(){
        ...
        if ($('#personname').val().length < 2 || $('#personname').val().length > 30) { Whatsappmanager.displaywarning("Name field should min 2 to max 30 characters"); }
        else{if ($('#password').val().length < 8 || $('#password').val().length > 30) { Whatsappmanager.displaywarning("Password Should be 8 to 30 Characters"); }
        else{
        if ($('#personcontact_number').val().length < 8 || $('#personcontact_number').val().length > 14) { Whatsappmanager.displaywarning("Contact Number Should be 8 to 14 digit"); $('.edit_manager').show(); }
        else{
        var self=this;
        var data_id=$(this).attr('data-id');
                var formData={
            ...
            $.ajax({
                url: Whatsappmanager.base_url+"Whatsapp_manager/edit_perticular_waitinglist_manager",
                type:'POST',
                data:formData ,
                success: function(result){
                   if (result.status) {
                        ...
                        $('.edit_manager').hide();
                        Whatsappmanager.displaysucess("Information Save successfully");
                       Whatsappmanager.listmanager(data);
                   }
                   else{
                        Whatsappmanager.displaywarning("Something went wrong please try again");
            ...
            Whatsappmanager.displaywarning("Please Fill all the fields");
        }
    },
    ...
    onDeleteManager:function(){
        ...
        var text="Delete Whatsapp Manager";
        swal({
            title: title,
            text: text,
            type: 'warning',
            ...
            $.ajax({
                url: Whatsappmanager.base_url+"Whatsapp_manager/delete_perticular_waitinglist_manager",
                type:'POST',
                data:formData ,
                success: function(result){
                   if (result.status) {
                        Whatsappmanager.displaysucess("Delete successfully");
                        var data={
                            per_page:$('.btn-per-page').attr('selected-per-page'),
                            page:1
                        }
    ...
    onSaveGroupname:function(){
        if($('#name').val()!="" && $('#email').val()!="" && $('#contact_number').val()!=""){
            if ($('#contact_number').val().length < 8 || $('#contact_number').val().length > 14) { Whatsappmanager.displaywarning("Contact Number Should be 8 to 14 digit"); }
            else{
            if ($('#name').val().length < 2 || $('#name').val().length > 30) { Whatsappmanager.displaywarning("Name field should min 2 to max 30 characters"); }
            else{
            ...
            if ($('#profile_photo').val() != '') {
                $('#image-loader').show();
                $.ajax({
                    url: Whatsappmanager.base_url+"Whatsapp_manager/add_profile_photo",
                    type:'POST',
                    dataType: 'json',
                    data: {
                        name : $('#name').val(),
                    ...
                    success: function(result){
                        if (result.status) {
                            ...
                            if(result.is_email_exist){
                                ...
                                $('#contact_number').val('');
                                //$('#profile_photo').val('');
                                Whatsappmanager.displaywarning("Email already exist.");
                            }else{
                                Whatsappmanager.displaysucess("User created successfully");
                                var data={
                                    per_page:$('.btn-per-page').attr('selected-per-page'),
                                    page:1
                                }
                ...
                $('#image-loader').show();
                $.ajax({
                    url: Whatsappmanager.base_url+"Whatsapp_manager/save_waiting_manager",
                    type:'POST',
                    dataType: 'json',
                    data: {
                        name : $('#name').val(),
            ...
            Whatsappmanager.displaywarning("Invalid Email");
        }
        }
        }
        }
    ...
    listmanager:function(data,fromevent){
        $.ajax({
            url: Whatsappmanager.base_url+"Whatsapp_manager/list_manager/",
            type:'POST',
            dataType: 'json',
            data: data,
                    ...
                    html+='<tr menu-id="'+managers[i].id+'">\
                            ...
                            if(managers[i].profile_photo==null || managers[i].profile_photo == ''){
                                html+='<td title="Browse">\
                                    ...
                                    <img class="img-upload rounded-circle"  src="'+Whatsappmanager.base_url+'assets/images/users/user.png" style="height:50px;width:50px;">\
                                    </td>';
                            }
                            else{
                                 html+='<td title="Browse">\
                                    ...
                                    if(managers[i].profile_photo=="assets/images/users/user.png")
                                        html+='<img class="img-upload rounded-circle" src="'+Whatsappmanager.base_url+managers[i].profile_photo+'" style="height:50px;width:50px;"></td>';
                                    else
