var Whatsappcust ={
    ...
    bind_events :function() {
        ...
        $('.a-recipe-perpage').on('click',function(){
            ...
            Whatsappcust.listCustomers(data);
        });
        ...
        $('#searchInput').on('keyup',function(){
             ...
             if($(this).val()==""){
                ...
                Whatsappcust.listCustomers(data,'fromsearch');
            }else{
    ...
    uploadexlfile:function(){
        ...
        $.ajax({
            url: Whatsappcust.base_url+"whatsapp_manager/uploadData",
            type:'POST',
            data: $form_data,
            processData:false,
            contentType:false,
            ...
            success: function(result){
                ...
                $('#image-loader').show();
                if (result.status) {
                    Whatsappcust.displaysucess("upload successfully");
                } 
                else{
                    Whatsappcust.displaywarning(result.msg);
                }
                $('#image-loader').hide();
                var data={
                    per_page:$('.btn-per-page').attr('selected-per-page'),
    ...
    onEditPerticularManager:function(){
        ...
        if ($.trim($('#personname').val()).length < 2 || $.trim($('#personname').val()).length > 30) { 
            Whatsappcust.displaywarning("Name field should min 2 to max 30 characters"); return false; }
        if ($('#personcontact_number').val().length < 8 || $('#personcontact_number').val().length > 14) { 
            Whatsappcust.displaywarning("Contact Number Should be 8 to 14 digit"); $('.edit_manager').show(); return false; }
        var self=this;
        var data_id=$(this).attr('data-id');
            ...
            $.ajax({
                url: Whatsappcust.base_url+"whatsapp_manager/edit_perticular_customer",
                type:'POST',
                data:formData ,
                success: function(result){
                   if (result.status) {
                        ...
                        $('.edit_manager').hide();
                        Whatsappcust.displaysucess("Information Save successfully");
                        Whatsappcust.listCustomers(data);
                   }
                   else{
                        if (result.msg) {
                        Whatsappcust.displaywarning("Contact Number already exist");}
                        else{
                            Whatsappcust.displaywarning("Something went wrong");
                        }
                   }
                }
            });
            ...
            Whatsappcust.displaywarning("Please Fill all the fields");
        }
    },
    ...
    onEditManager:function(){
        ...
        var formData={
            ...
            $.ajax({
                url: Whatsappcust.base_url+"whatsapp_manager/show_perticular_customer",
                type:'POST',
                data:formData ,
                success: function(result){
                        var data={
                            ...
                            <form class="form-recipe-edit" method="post" action="javascript:;">\
                                <div class="row">\
                                    ...
                                    <div class="col-lg-4 col-md-4 col-sm-12 col-12" style="margin-top:10px;">\
                                        ...
                                        <input type="text" name="personcontact_number" id="personcontact_number" value="'+result.contact_no+'" class="form-control" placeholder="Enter Contact Number" onkeypress="return Whatsappcust.isNumber(event)">\
                                    </div>\
                                </div>\
                                <div class="row">\
    ...
    onSaveGroupname:function(){
        if($('#name').val()!="" && $('#contact_number').val()!=""){
            if ($('#contact_number').val().length < 8 || $('#contact_number').val().length > 14) { 
                Whatsappcust.displaywarning("Contact Number Should be 8 to 14 digit"); return false; }
            ...
            if (regexEmail.test(email.value) == false) { 
                Whatsappcust.displaywarning("Invalid Email");
                return false;
            }
            }
                    $('#image-loader').show();
                    $.ajax({
                        url: Whatsappcust.base_url+"whatsapp_manager/save_customers",
                        type:'POST',
                        dataType: 'json',
                        data: {
                            name : $('#name').val(),
                        ...
                        success: function(result){
                            ...
                            if (result.status) {
                                if(result.is_email_exist){
                                    ...
                                    Whatsappcust.displaysucess("Customer created successfully");
                                    var data={
                                        per_page:$('.btn-per-page').attr('selected-per-page'),
                                        page:$('.btn-current-pageno').attr('curr-page')
                                    }
        ...
        Whatsappcust.displaywarning("Customer name and mobile number should not be empty");
    }
    ...
    deleteGroup:function(){
        if($(this).is(':checked')){
            $(this).val("on");
            Whatsappcust.displaysucess("Customer is Unblock now");
        }

        else{
            $(this).val("off");
            Whatsappcust.displaysucess("Customer is Block now");
        }
        ...
        $('#image-loader').show();
        $.ajax({
            url: Whatsappcust.base_url+"whatsapp_manager/block_unblock_customer",
            type:'POST',
            data:formData,
            success: function(result){
                $('#image-loader').hide();
                if (result.status) {
                    ...
                    Whatsappcust.displaywarning("Something went wrong please try again");
                }
            }
        });
        /*swal({
    ...
    listCustomers:function(data,fromevent){
        $('#image-loader').show();
        $.ajax({
            url: Whatsappcust.base_url+"whatsapp_manager/list_customers/",
            type:'POST',
            dataType: 'json',
            data: data,
