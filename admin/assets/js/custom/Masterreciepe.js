var Masterreciepe ={
    ...
    init:function() {
        if(Masterreciepe.create_type=="barmenu")
            Masterreciepe.main_menu_id=2;
        else
            Masterreciepe.main_menu_id=1;
        this.bind_events();
    ...
    bind_events :function() {
        ...
        $('#imgupload').on('change',function(event){
           /* Masterreciepe.displaywarning('File requirement: JPG, PNG up to 1MB. Minimum pixels required: 500 for width, 300 for height.');*/
            if($('#imgupload').val()==""){
            ...
            if($.inArray(ext, ['jpg','jpeg','png']) == -1) {
                Masterreciepe.displaywarning('invalid extension!');
                return false;
            }
            ...
            if (window.File && window.FileList && window.FileReader) { 
                        ...
                        swal({
                            ...
                            if(confirm){
                               ...
                               imageData = canvas.toDataURL('image/jpeg',0.8);
                                ...
                                $form_data.append('id', Masterreciepe.recipe_id);
                                $.ajax({
                                    url: Masterreciepe.base_url+"Menumaster/update_recipe_image",
                                    type:'POST',
                                    data: $form_data,
                                    processData:false,
                                    contentType:false,
                                    ...
                                    success: function(result){
                                        ...
                                        if (result.status) { 
                                            ...
                                            if(result.msg){
                                                Masterreciepe.displaywarning(result.msg);
                                            }
                                            else
                                                Masterreciepe.displaywarning("Something went wrong please try again");
                                        }
                                         $("#cropbox").attr('src','');
                                        $('#image-loader').hide();
                ...
                Masterreciepe.displaywarning("Your browser does not support File API");  
                $("#cropbox").attr('src','');
                $('#image-loader').hide();
                $('#OpenImgUpload').removeAttr('disabled');
                $('#OpenImgUpload').css('pointer-events','all');
        ...
        $("#crop").click(function(){
            if(!Masterreciepe.size){
                Masterreciepe.displaywarning("Please crop the image.");
                return false;
            }
            if($("#cropbox").attr('src')==""){
                Masterreciepe.displaywarning("Please crop the image.");
            ...
            $form_data.append('x', Masterreciepe.size.x);
            $form_data.append('y', Masterreciepe.size.y);
            $form_data.append('w', Masterreciepe.size.w);
            $form_data.append('h', Masterreciepe.size.h);
        ...
        $(".a-add-ingredient").on('click',function(){
            if($('.input-recipe-name').val()=="" || $('.input-group-name').val()==""){
                ...
                Masterreciepe.displaywarning("Please add group name and recipe name.");
                return false;
            }
        ...
        $('.div-ingredient-result').on('mouseover','.lv-item',function(){
            ...
            if(item_no){
                ...
                if($('#info_'+item_no+'_container').length == 0)
                    Masterreciepe.showNutrientInformation($(this).attr('data-add-ingredient'));
                else
        ...
        $('.input-search-ingredient').on('keyup',function(){
            if($(this).val()=="")
                $('.row-msg-enter').show();
            Masterreciepe.searchIngredients($('.input-search-ingredient').val(),1);
        });

        $('.div-ingredient-result').on('click','.a-page-ingredient',function(){
            Masterreciepe.searchIngredients($(this).attr('query'),$(this).attr('page-no'));
        });
        ...
        $('.div-ingredient-result').on('click','.ingredient-add-item',function(){
            ...
            $('#image-loader').show();
            Masterreciepe.saveIngredientItems(ingredient_id,data);
        });
        ...
        $('#current_ingredients_list').on('change','.ingredient_quantity_box',function(){
            ...
            Masterreciepe.updateIngredientItems(data);
            Masterreciepe.calculateAllTotal();
            $('#net_weight').trigger('change');
        });
        
        $('.input-recipe-name').on('change',this.updateRecipes);
        $('.input-recipe-desc').on('change',this.updateRecipes);
        $('.input-recipe-decnm').on('change',this.updateRecipes);
    ...
    deleteRecipePrice:function(){
        ...
        var seq=$(this).closest('.row').attr('sequence');
         $.ajax({
            url: Masterreciepe.base_url+"Menumaster/delete_recipe_price/",
            type:'POST',
            dataType: 'json',
            data: {'sequence':seq,'recipe_id':Masterreciepe.recipe_id},
            success: function(result){
    ...
    saveRecipeHeader:function(){
        if($('.input-recipe-name').val()==""){
            Masterreciepe.displaywarning("please enter recipe name");
            $('.input-recipe-name').focus();
            $('.input-recipe-name').val(Masterreciepe.recipe_name);
            return false;
        }
        if($('#in-recipe-quantity').val()=="" || $('#in-recipe-price').val()==""){
            Masterreciepe.displaywarning("Please add price and quantity both");
            $('#in-recipe-quantity').focus();
            return false;
        }
        if($('.input-group-name').val()==""){
            Masterreciepe.displaywarning("please enter group name");
       ...
       if($('.input-recipe-name').val()!="" && $('.input-group-name').val()!=""){
        ...
        $('.form-recipe-header [name="main_menu_id"]').val(Masterreciepe.main_menu_id);
        $.ajax({
            url: Masterreciepe.base_url+"Menumaster/save_recipe_header/",
            type:'POST',
            dataType: 'json',
            ...
            success: function(result){
                var data=result.recipe;
                if (result.status) { 
                    Masterreciepe.displaysucess("Information saved successfully.");
                }else{
                    Masterreciepe.displaywarning(result.msg);
                }
            }
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                ...
                $('#cropbox').Jcrop({
                    ...
                    backdrop: 'static',
                    onSelect: function(c){
                       Masterreciepe.size = {x:c.x,y:c.y,w:c.w,h:c.h};   
    ...
    updateRecipes:function(data){
        ...
        var data={
            id : Masterreciepe.recipe_id,
            name:$('.input-recipe-name').val(),
            long_desc:$('.input-recipe-desc').val(),
            declaration_name:$('.input-recipe-decnm').val(),
            ...
            alacalc_recipe_id:Masterreciepe.alacalc_recipe_id,
            group_id:$('.input-group-id').val()
        }
        $.ajax({
            url: Masterreciepe.base_url+"Menumaster/update_recipe/",
            type:'POST',
            dataType: 'json',
            data: data,
            success: function(result){
    ...
    updateIngredientItems:function(data){
        data+="&recipe_id="+Masterreciepe.recipe_id;
        data+="&alacalc_recipe_id="+Masterreciepe.alacalc_recipe_id;

        $.ajax({
            url: Masterreciepe.base_url+"Menumaster/update_ingredient_items/",
            type:'POST',
            dataType: 'json',
            data: data,
            success: function(result){
    ...
    onDeleteItem:function(){
        ...
        swal({
            ...
            var formData = {
                ...
                recipe_id:Masterreciepe.recipe_id,
                alacalc_recipe_id:Masterreciepe.alacalc_recipe_id,
                alacalc_item_id:alacalc_item_id
            };
            $.ajax({
                url: Masterreciepe.base_url+"Menumaster/delete_ingredient_item",
                type:'POST',
                data:formData ,
                success: function(result){
                   if (result.status) {
                        $('#current_ingredients_list').html('');
                       Masterreciepe.showCurrentIngredient(result['ingredient_items']);
                   }
                   else{
                        Masterreciepe.displaywarning("Something went wrong please try again");
                   }
    ...
    loadRecipeAlacalc:function(){
        $.ajax({
            url: Masterreciepe.base_url+"aalcalc/get_recipe/"+Masterreciepe.alacalc_recipe_id,
            type:'POST',
            dataType: 'json',
            data: {},
            success: function(response){
                ...
                if (response.status) { 
                   ...
                   Masterreciepe.loadNutrition(nutrition,result.recipes);
                   Masterreciepe.loadAllergens(result.linked.ingredient_items,result.linked.allergens,result.recipes.quantity_per_serving);
    ...
    {    
        ...
        for (k in ingredient_items) {
            ...
            if(!Masterreciepe.c_from){
                $('.ul-allgerens').append(html);
            }else{
                $('.ul-allgerens.'+Masterreciepe.alacalc_recipe_id).append();
            }
            if(str=="")
          ...
          /*  if(allergens_str!="")
                ...
                $('.p-allergens-li').append(str);*/
                 ...
                 if(allergens_str!="")
                $('.p-allergens-li.'+Masterreciepe.alacalc_recipe_id).append('<ul><li><b style="font-weight: 800;">'+allergens_str+'</b></li>');
            else
                $('.p-allergens-li.'+Masterreciepe.alacalc_recipe_id).append('</ul>');
            }
    ...
    loadNutrition:function(data,recipe_details){
        ...
        if(!Masterreciepe.c_from){
            ...
            $('.div-nutrition.'+Masterreciepe.alacalc_recipe_id).append(html);
        }
    },
    loadRecipe:function(){
        $.ajax({
            url: Masterreciepe.base_url+"Menumaster/get_recipe/"+Masterreciepe.recipe_id,
            type:'POST',
            dataType: 'json',
            data: {},
            success: function(result){
                ...
                if (result.status) { 
                    ...
                    Masterreciepe.recipe_name=data['name'];
                    Masterreciepe.recipe_price=data['price'];
                    if(data['name']=="" || data['group_id']=="" || data['group_id']==null || data['name']==null){
                        $('.nutrition a').css('pointer-events','none');
                        $('.nutrition a').css('cursor','no-drop');
                    }
                    if(Masterreciepe.is_category_prices==1){
                        var table_categories=Masterreciepe.table_categories;
                     ...
                     if(data.recipe_image=="" || data.recipe_image=="assets/images/users/menu.png"){
                        $('.recipe-image-upload').attr('data-image-src',Masterreciepe.base_url+'assets/images/users/menu.png');
                        $(".recipe-image-upload").css("background-image", "url(" + Masterreciepe.base_url+'assets/images/users/menu.png' + ")");
                     }else{
                        $('.recipe-image-upload').attr('data-image-src',data.recipe_image);
                        $(".recipe-image-upload").css("background-image", "url(" + data.recipe_image + ")");
                     }

                    if(Masterreciepe.is_nutrition==0){
                        if(data['ingedient_items'].length>0){
                            Masterreciepe.showCurrentIngredient(data['ingedient_items']);
                    ...
                    if(Masterreciepe.is_alacalc_recipe==1){
                        Masterreciepe.loadNutrition(nutrition,data);
                        Masterreciepe.loadAllergens(data.ingedient_items,data.allergens,data.quantity_per_serving);
                        if(!Masterreciepe.c_from){
                            ...
                            $('.p-allergens-li.'+Masterreciepe.alacalc_recipe_id).append('<ul><li>'+data.declaration_name+'</li></ul>');
                    }
                    }
    ...
    onChangeGroupName:function(){
       if($('.input-recipe-name').val()!="" && $('.input-group-name').val()!=""){
        ...
        if($('.input-group-name').val()!=""){
                ...
                $.ajax({
                    url: Masterreciepe.base_url+"Menumaster/save_menu_group",
                    type:'POST',
                    dataType: 'json',
                    data: {group_name : $('.input-group-name').val(),recipe_id:Masterreciepe.recipe_id,group_id:$('.input-group-id').val(),main_menu_id:Masterreciepe.main_menu_id},
                    success: function(result){
                        if (result.status) { 
    ...
    onAddGroupName:function(){
        ...
        if($('.input-recipe-type').val()==""){
            Masterreciepe.displaywarning("please enter recipe type");
            $('.input-recipe-type').focus();
            $('.input-recipe-type').val(Masterreciepe.recipe_type);
            return false;
        }
        if($('.input-group-name').val()==""){
            Masterreciepe.displaywarning("please enter group name");
        ...
        if($('.input-recipe-price').val()==""){
            Masterreciepe.displaywarning("Please enter recipe price");
        ...
        if(Masterreciepe.is_category_prices==1){
            ...
            if(cat_type==1){
                Masterreciepe.displaywarning("Table category should not be empty.");
                return false;
            }
            if(flag_price==1){
                Masterreciepe.displaywarning("price should not be empty.");
                return false;
            }
            var cate_duplicate = [];
            ...
            if(cate_duplicate.length!=0){
                Masterreciepe.displaywarning("Can not add different prices for same category ");
                return false;
            }
        }
       if($('.input-recipe-name').val()!="" && $('.input-group-name').val()!=""){
        ...
        if($('.input-group-name').val()!=""){
            ...
            data+="&best_time_to_eat="+$('.input-timeto-eat').val();
          /*  data={group_name : $('.input-group-name').val(),recipe_id:Masterreciepe.recipe_id,best_time_to_eat:$('.input-timeto-eat').val(),main_menu_id:Masterreciepe.main_menu_id};*/
            $.ajax({
                url: Masterreciepe.base_url+"Menumaster/save_menu_group",
                type:'POST',
                dataType: 'json',
    ...
    saveIngredientItems:function(ingredient_id,ingredients){
        var ingredients=Masterreciepe.ingredients_data[ingredient_id];
        if(ingredients){
            $('.recipe-add-ingredient').toggle();
            $('.recipe-overview').toggle();
            Masterreciepe.showIngredientHTML(ingredients);
            var data={
            ...
            Masterreciepe.saveRecipeItems(data);
        }else{
            $.ajax({
                url: Masterreciepe.base_url+"aalcalc/get_ingredient_data/"+ingredient_id,
                type:'POST',
                dataType: 'json',
                data: {},
                success: function(result){
    ...
    saveRecipeItems:function(data){
        $.ajax({
            url: Masterreciepe.base_url+"Menumaster/save_recipe_items/",
            type:'POST',
            dataType: 'json',
            data: data,
            success: function(result){
